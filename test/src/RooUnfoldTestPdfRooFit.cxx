...
Int_t mcgenpdf2(const RooAbsPdf &pdf, RooRealVar &xvar, Int_t nx, TVectorD &x, TH1 *h = 0, Double_t offset = 0.0,
                Int_t verbose = 0)
{
   ...
   if (h) {
      ...
      if (verbose >= 1)
         cout << "fill histogram " << h->GetName() << ": norm=" << norm << ", offset=" << offset << endl;
      ...
      if (verbose >= 1)
         cout << "histogram " << h->GetName() << " filled" << endl;
   }
   delete ds;
   return ngen;
}

Int_t mcgenpdf(const RooAbsPdf &sigpdf, RooRealVar &xvar, Int_t nx, TVectorD &x, TH1 *h = 0, Double_t bkg = 0.0,
               Double_t offset = 0.0, Int_t verbose = 0)
{
   if (bkg <= 0.0) {
      ...
      if (bkg >= 1.0)
         return mcgenpdf2(bkgpdf, xvar, nx, x, h, offset, verbose);
      RooRealVar fbkg("fbkg", "background fraction", bkg, 0.0, 1.0);
      TString pdfname(sigpdf.GetName());
      pdfname += "+bkg";
      return mcgenpdf2(RooAddPdf(pdfname, sigpdf.GetName(), RooArgList(bkgpdf, sigpdf), fbkg), xvar, nx, x, h, offset,
                       verbose);
   }
}
...
Int_t RooUnfoldTestPdf(Int_t fpdf, Int_t nx, Double_t xlo, Double_t xhi, TVectorD &x, TH1 *h = 0, Double_t mean = 0.0,
                       Double_t width = 2.5, Double_t bkg = 0.0, Int_t verbose = 0)
{
   using namespace RooFit;
   ...
   case 0: {
      return mcgenpdf2(RooChebychev("pdf", "constant", xvar, RooArgList()), xvar, nx, x, h, offset, verbose);
   }
   ...
   case 1: {
      ...
      return mcgenpdf(RooParametricStepFunction("pdf", "top hat", xvar, steplist, steps, stepbins), xvar, nx, x, h, bkg,
                      0.0, verbose);
   }
   ...
   case 2: {
      ...
      return mcgenpdf(RooGaussian("pdf", "Gaussian", xvar, p51, p52), xvar, nx, x, h, bkg, 0.0, verbose);
   }

   case 3: { // NB. xvar limits offset by mean above
      RooRealVar p21("p21", "tau", width);
      RooTruthModel dectm("dectm", "decay truth model", xvar);
      return mcgenpdf(RooDecay("pdf", "decay", xvar, p21, dectm, RooDecay::DoubleSided), xvar, nx, x, h, bkg, mean,
                      verbose);
   }
   ...
   case 4: {
      ...
      return mcgenpdf(RooBreitWigner("pdf", "Breit Wigner", xvar, p11, p12), xvar, nx, x, h, bkg, 0.0, verbose);
   }
   ...
   case 5: {
      ...
      return mcgenpdf(RooAddPdf("pdf", "Two Breit Wigners", RooArgList(bw1, bw2), RooArgList(fbw1)), xvar, nx, x, h,
                      bkg, 0.0, verbose);
   }
   ...
   case 6: {
      ...
      return mcgenpdf(RooExponential("pdf1", "Exponential", xvar, ptau), xvar, nx, x, h, bkg, 0.0, verbose);
   }
   ...
   case 7: {
      ...
      return mcgenpdf(RooAddPdf("pdf", "Resonace+background", RooArgList(expon, gaus), RooArgList(fexpvar)), xvar, nx,
                      x, h, bkg, 0.0, verbose);
   }
