...
void RooUnfoldTestHarness::Parms (ArgVars& args)
{
  ...
  methodHelp2.Form("%d=GP, %d=Poisson",
                   RooUnfold::kGP, RooUnfold::kPoisson);
  methodHelp += methodHelp2;
...
Int_t RooUnfoldTestHarness::Run()
{
  ...
  const TNamed* objs[] = { unfold, hTrain, hTrainTrue, hTrainFake, hTrue, hMeas, hFake,
                           hUnfold, hRes, hPulls, hPDFx, hTestPDFx,
...
Int_t RooUnfoldTestHarness::Unfold ()
{
  ...
  hCorr= CorrelationHist (unfold->Eunfold((RooUnfold::ErrorTreatment)doerror),
...
void RooUnfoldTestHarness::Results()
{
  ...
  if (hUnfold) hUnfold->Draw("SAME P");
...
void RooUnfoldTestHarness::Reset()
{
  ...
  ps= 0;
  hPDFx= hTestPDFx= 0;
  hTrain= hTrainTrue= hTrainFake= hTrue= hMeas= hFake= hUnfold= hRes= hPulls=
...
void RooUnfoldTestHarness::Legend (TLegend*& legend, TH1* pdf, TH1* truth, TH1* fake, TH1* meas, TH1* unfold, TF1* ff, TF1* tf)
{
