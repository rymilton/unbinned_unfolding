...
void RooUnfoldTestHarness::Parms(ArgVars &args)
{
   ...
   methodHelp.Form("unfolding method: %d=none, %d=Bayes, %d=SVD, ", RooUnfold::kNone, RooUnfold::kBayes,
                   RooUnfold::kSVD);
   methodHelp2.Form("%d=bin-by-bin, %d=TUnfold, %d=invert, %d=IDS, ", RooUnfold::kBinByBin, RooUnfold::kTUnfold,
                    RooUnfold::kInvert, RooUnfold::kIDS);
   methodHelp += methodHelp2;
   methodHelp2.Form("%d=GP, %d=Poisson", RooUnfold::kGP, RooUnfold::kPoisson);
   methodHelp += methodHelp2;
   ...
   args.Add("ftrainx", &ftrainx, 0,
            "training X PDF: 0=flat, 1=top-hat, 2=Gaussian, 3=double exp, 4=B-W, 5=double B-W, 6=exp, 7=Gaussian+exp");
   ...
   args.Add("overflow", &overflow, 0,
            "0=unfolding ignores under/overflow bins, 1=unfolding uses under/overflow bins, 2=fill under/overflows "
            "into first/last bins");
   args.Add("addbias", &addbias, -1,
            "apply systematic shift/rotation (disabled by default for bin-by-bin/invert methods)", "1");
   ...
   args.Add("ntoys", &ntoys, -1,
            "number of toys for error calculation (doerror=3) or plots (ploterrors=1, default ntoys=500)", "50");
   ...
   args.Add("seed", &seed, -1,
            "random number seed for test distributions and RooUnfold toy error calculation (use seed=0 for a different "
            "seed on each run)",
            "");
...
Int_t RooUnfoldTestHarness::Run()
{
   ...
   const TNamed *objs[] = {unfold,  hTrain,  hTrainTrue, hTrainFake, hTrue,     hMeas,    hFake,     hUnfold,
                           hRes,    hPulls,  hPDFx,      hTestPDFx,  hResmat,   hCorr,    hMeasCorr, hUnfErr,
                           hToyErr, fitFunc, trueFunc,   ntChi2,     hParmChi2, hParmErr, hParmRes,  hParmRms};
   for (size_t i = 0; i < sizeof(objs) / sizeof(objs[0]); i++) {
      if (objs[i]) {
         if (verbose >= 2)
            std::cout << "Write object '" << objs[i]->GetName() << "'" << std::endl;
...
void RooUnfoldTestHarness::SetupCanvas()
{
   ...
   if (onepage) {
      if (wpaper > 0)
         gStyle->SetCanvasDefW(wpaper);
      if (hpaper > 0)
         gStyle->SetCanvasDefH(hpaper);
...
void RooUnfoldTestHarness::TrainResults()
{
   if (mscalex != 1.0) {
      ...
      if (hTrainFake)
         hTrainFake->Scale(mscalex, "nosw2");
   }
   setmax(hTrainTrue, hPDFx, hTrain, hTrainFake);

   if (onepage && (onepage < 4 || ipad >= onepage))
      return;
   if (onepage)
      canvas->cd(++ipad);
   hTrainTrue->Draw();
   ...
   hTrain->Draw("SAME");
   if (hTrainFake)
      hTrainFake->Draw("SAME");
   if (!onepage)
      Legend(lTrain, hPDFx, hTrainTrue, hTrainFake, hTrain);
   canvas->Update();
}
...
void RooUnfoldTestHarness::SetMeasuredCov()
{
   ...
   hMeasCorr =
      CorrelationHist(cov, "measCor", "Measured correlation matrix", response->Hresponse()->GetXaxis()->GetXmin(),
...
Int_t RooUnfoldTestHarness::Unfold()
{
   ...
   if (ntoys < 0)
      ntoys = (ploterrors) ? 500 : 50;
   unfold->SetNToys(ntoys);
   unfold->IncludeSystematics(dosys);
   SetMeasuredCov();

#ifdef USE_TUNFOLD_H
   if (method == RooUnfold::kTUnfold)
      (dynamic_cast<RooUnfoldTUnfold *>(unfold))->SetRegMethod((TUnfold::ERegMode)regmethod);
#endif
   if (verbose >= 0) {
      cout << "Created ";
      unfold->Print();
   }
   hUnfold = unfold->Hunfold((RooUnfold::ErrorTreatment)doerror);
   if (!hUnfold)
      return 0;
   // hUnfold->SetName("unfold");
   hUnfold->SetLineColor(kBlack); // otherwise inherits style from hTrainTrue
   if (verbose >= 0)
      unfold->PrintTable(cout, hTrue, (RooUnfold::ErrorTreatment)doerror);
   ...
   hRes = dynamic_cast<TH1 *>(hUnfold->Clone("res"));
   hRes->Reset();
   hRes->SetTitle("Residuals");
   hPulls = dynamic_cast<TH1 *>(hUnfold->Clone("pulls"));
   hPulls->Reset();
   hPulls->SetTitle("Pulls");

   for (Int_t i = 1; i <= ntbins; i++) {
      if ((hUnfold->GetBinContent(i) != 0.0 || (doerror && hUnfold->GetBinError(i) > 0.0)) &&
   ...
   hCorr = CorrelationHist(unfold->Eunfold((RooUnfold::ErrorTreatment)doerror), "corr", "Unfolded correlation matrix",
                           response->Hresponse()->GetYaxis()->GetXmin(), response->Hresponse()->GetYaxis()->GetXmax());
   return 1;
}
...
void RooUnfoldTestHarness::Results()
{
   if (mscalex != 1.0) {
      ...
      if (hFake)
         hFake->Scale(mscalex, "nosw2");
      if (fitFunc)
         fitFunc->GetHistogram()->Scale(mscalex, "nosw2");
   }
   setmax(hTrue, hTestPDFx, hMeas, hFake);
   if (hUnfold) {
      hUnfold->SetMarkerStyle(kFullDotLarge);
      setmax(hTrue, hUnfold);
   }

   if (onepage && ipad >= onepage)
      return;
   if (onepage)
      canvas->cd(++ipad);
   hTrue->Draw();
   ...
   if (fitFunc)
      fitFunc->GetHistogram()->Draw("SAME");
   if (trueFunc)
      trueFunc->Draw("LSAME");
   hMeas->Draw("SAME");
   if (hFake)
   ...
   if (hUnfold)
      hUnfold->Draw("SAME P");
   Legend(lTest, hTestPDFx, hTrue, hFake, hMeas, hUnfold, fitFunc, trueFunc);
   canvas->Update();
...
void RooUnfoldTestHarness::PlotParms()
{
   ...
   if (minparm != -1e30)
      parms->SetMinParm(minparm);
   if (stepsize > 0)
      parms->SetStepSizeParm(stepsize);
...
TF1 *RooUnfoldTestHarness::FitFunc(Int_t fpdf, TH1 *h, Double_t mean, Double_t width, Double_t bkg)
{
   ...
   case 1: {
      ...
      f = new TF1(
         "trueFunc",
         Form("[0]*(ROOT::Math::gaussian_cdf((x-[2]+[3])/(%g)) - ROOT::Math::gaussian_cdf((x-[2]-[3])/(%g))) + [1]", xw,
              xw),
         xlo, xhi);
   ...
   default: cerr << "PDF " << fpdf << " not defined" << endl;
   }
   //  if (f) f->Print();
   return f;
}
...
TH1D *RooUnfoldTestHarness::Generate(TVectorD &x, const char *name, const char *title, Int_t nt, Int_t fpdf, Int_t nx,
                                     Double_t lo, Double_t hi, Double_t mean, Double_t width, Double_t bkg,
                                     Double_t /* bias */, Double_t /* smear */)
{
   ...
   if (verbose >= 1)
      cout << "Generate values in range " << lo << " to " << hi << endl;
   if (!RooUnfoldTestPdf(fpdf, nx, lo, hi, x, hPDF, mean, width, bkg, verbose))
      return 0;
...
Int_t RooUnfoldTestHarness::Fakes(TVectorD &x, Int_t nm, Int_t nx, Double_t x0, Double_t x1, Double_t y0,
                                  Double_t y1) const
{
   if (!addfakes || (y0 <= 0.0 && y1 <= 0.0))
      return 0;
   if (overflow == 1) {
      // Fill under/overflow bins too
      Double_t xw = x1 - x0;
   ...
   if (yw == 0.0) {
      ...
      if (nx <= 0)
         return 0;
      if (verbose >= 1)
         cout << "Generate " << nx << " fakes in range " << x0 << " to " << x1 << endl;
      x.ResizeTo(nx);
      ...
      for (Int_t i = 0; i < nx; i++) {
         ...
         Double_t xv;
         if (xp >= x0 && xp < x1)
            xv = xp; // which root to take? Maybe always this +ve one.
         else if (xn >= x0 && xn < x1)
            xv = xn;
         else
            continue;
      ...
      if (nnx < nx) {
         if (verbose >= 0)
            cout << "Trouble generating " << nx - nnx << " fakes (of " << nx << ")" << endl;
         nx = nnx;
         x.ResizeTo(nx);
      }
   }
...
Double_t
RooUnfoldTestHarness::Smear(Double_t x, Int_t nt, Double_t lo, Double_t hi, Double_t bias, Double_t smear) const
{
...
Double_t RooUnfoldTestHarness::Overflow(Double_t x, Int_t nb, Double_t lo, Double_t hi) const
{
   if (overflow != 2)
      ...
      return lo + (hi - lo) / Double_t(2 * nb);
   else if (!(x < hi))
      return hi - (hi - lo) / Double_t(2 * nb);
   else
      return x;
}
...
RooUnfoldTestHarness::RooUnfoldTestHarness(const char *name) : TNamed(name, name)
{
   Reset();
   SetDefaults();
}

RooUnfoldTestHarness::RooUnfoldTestHarness(const char *name, int argc, const char *const *argv) : TNamed(name, name)
{
   Reset();
   error = SetArgs(argc, argv);
}

RooUnfoldTestHarness::RooUnfoldTestHarness(const char *name, const char *args) : TNamed(name, name)
{
   Reset();
...
void RooUnfoldTestHarness::Reset()
{
   ...
   ps = 0;
   hPDFx = hTestPDFx = 0;
   hTrain = hTrainTrue = hTrainFake = hTrue = hMeas = hFake = hUnfold = hRes = hPulls = hUnfErr = hToyErr = hParmChi2 =
      hParmErr = hParmRes = hParmRms = 0;
...
{
   if (addbias == -1)
      addbias = (method == RooUnfold::kBinByBin || method == RooUnfold::kInvert)
                   ? 0
                   : 1; // bin-by-bin/invert methods can't handle bias

   if (verbose >= 0)
      PrintParms(cout);

   if (xlo >= xhi) {
      cout << "Error: xlo (" << xlo << ") >= xhi(" << xhi << ")" << endl;
      error = 2;
   }
   if (ntest <= 0) {
      cout << "Error: ntest (" << ntest << ") <= 0" << endl;
      error = 2;
   }
   if (ntrain <= 0) {
      cout << "Error: ntrain (" << ntrain << ") <= 0" << endl;
      error = 2;
   }
   if (nmx <= 0) {
      cout << "Error: nmx (" << nmx << ") <= 0" << endl;
      error = 2;
   }
   if (ntx <= 0) {
      cout << "Error: ntx (" << ntx << ") <= 0" << endl;
      error = 2;
   }
   if (ftestx < 0) {
      cout << "Error: ftestx (" << ftestx << ") < 0" << endl;
      error = 2;
   }
   if (ftrainx < 0) {
      cout << "Error: ftrainx (" << ftrainx << ") < 0" << endl;
      error = 2;
   }
   return error;
}
...
TH2D *RooUnfoldTestHarness::CorrelationHist(const TMatrixD &cov, const char *name, const char *title, Double_t lo,
                                            Double_t hi)
{
...
void RooUnfoldTestHarness::Legend(TLegend *&legend, TH1 *pdf, TH1 *truth, TH1 *fake, TH1 *meas, TH1 *unfold, TF1 *ff,
                                  TF1 *tf)
{
   legend = new TLegend(0.79, (tf ? 0.62 : unfold ? 0.72 : 0.75), 0.894, 0.89);
   ...
   if (unfold)
      legend->AddEntry(unfold, "unfolded", "P");
   if (ff)
...
void RooUnfoldTestHarness::setmax(TH1 *h, const TH1 *h1, const TH1 *h2, const TH1 *h3, const TH1 *h4, const TH1 *h5,
                                  const TH1 *h6)
{
   ...
   if (h2 && h2->GetMaximum() > maxval)
      maxval = h2->GetMaximum();
   if (h3 && h3->GetMaximum() > maxval)
      maxval = h3->GetMaximum();
   if (h4 && h4->GetMaximum() > maxval)
      maxval = h4->GetMaximum();
   if (h5 && h5->GetMaximum() > maxval)
      maxval = h5->GetMaximum();
   if (h6 && h6->GetMaximum() > maxval)
      maxval = h6->GetMaximum();
   h->SetMinimum(0.0);
