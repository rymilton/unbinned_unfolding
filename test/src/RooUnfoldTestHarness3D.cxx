...
void RooUnfoldTestHarness3D::TrainResults()
{
   if (mscalex != 1.0) {
      ...
      if (hTrainFakeX)
         hTrainFakeX->Scale(mscalex, "nosw2");
   }
   if (mscaley != 1.0) {
      hTrainY->Scale(mscaley, "nosw2");
      if (hTrainFakeY)
         hTrainFakeY->Scale(mscaley, "nosw2");
   }
   if (mscalez != 1.0) {
      hTrainZ->Scale(mscalez, "nosw2");
      if (hTrainFakeZ)
         hTrainFakeZ->Scale(mscalez, "nosw2");
   }
   ...
   if (onepage && (onepage < 6 || ipad >= onepage))
      return;
   if (onepage)
      canvas->cd(++ipad);
   hTrainTrueX->Draw();
   ...
   if (!onepage)
      Legend(lTrain, hPDFx, hTrainTrueX, hTrainFakeX, hTrainX);
   canvas->Update();
...
void RooUnfoldTestHarness3D::Results()
{
   if (mscalex != 1.0) {
      ...
      if (hFakeX)
         hFakeX->Scale(mscalex, "nosw2");
   }
   if (mscaley != 1.0) {
      hMeasY->Scale(mscaley, "nosw2");
      if (hFakeY)
         hFakeY->Scale(mscaley, "nosw2");
   }
   if (mscalez != 1.0) {
      hMeasZ->Scale(mscalez, "nosw2");
      if (hFakeZ)
         hFakeZ->Scale(mscalez, "nosw2");
   }
   ...
   if (hUnfold) {
      hUnfoldX = ProjectionX(hUnfold, "hUnfoldX", "Unfolded X", "E");
      hUnfoldY = ProjectionY(hUnfold, "hUnfoldY", "Unfolded Y", "E");
      hUnfoldZ = ProjectionZ(hUnfold, "hUnfoldZ", "Unfolded Z", "E");
      hUnfoldXY = Projection2D(hUnfold, "xy", "hUnfoldXY", "Unfolded XY");
      hUnfoldYZ = Projection2D(hUnfold, "yz", "hUnfoldYZ", "Unfolded YZ");
      hUnfoldXZ = Projection2D(hUnfold, "xz", "hUnfoldXZ", "Unfolded XZ");
      hUnfoldXY->SetMarkerColor(kRed);
      hUnfoldYZ->SetMarkerColor(kRed);
      hUnfoldXZ->SetMarkerColor(kRed);
      hUnfoldX->SetMarkerStyle(kFullDotLarge);
      hUnfoldY->SetMarkerStyle(kFullDotLarge);
      hUnfoldZ->SetMarkerStyle(kFullDotLarge);
      setmax(hTrueX, hUnfoldX);
      setmax(hTrueY, hUnfoldY);
      setmax(hTrueZ, hUnfoldZ);
   }
   ...
   if (hUnfoldX)
      hUnfoldX->Draw("SAME");
   Legend(lTest, hTestPDFx, hTrueX, hFakeX, hMeasX, hUnfoldX);
   canvas->Update();
   ...
   for (Int_t ix = 1; ix <= ntx; ix++) {
      for (Int_t iy = 1; iy <= nty; iy++) {
         for (Int_t iz = 1; iz <= ntz; iz++) {
            if (hUnfold->GetBinContent(ix, iy, iz) != 0.0 && hUnfold->GetBinError(ix, iy, iz) > 0.0 &&
                (hTrue->GetBinContent(ix, iy, iz) != 0.0 || (doerror && hTrue->GetBinError(ix, iy, iz) > 0.0))) {
               Double_t pull = (hUnfold->GetBinContent(ix, iy, iz) - hTrue->GetBinContent(ix, iy, iz)) /
                               hUnfold->GetBinError(ix, iy, iz);
               hPullsX->Fill(hUnfold->GetXaxis()->GetBinCenter(ix), pull);
               hPullsY->Fill(hUnfold->GetYaxis()->GetBinCenter(iy), pull);
               hPullsZ->Fill(hUnfold->GetZaxis()->GetBinCenter(iz), pull);
            }
         }
      }
   }
   ...
   if (onepage && (ipad >= onepage || ploterrors))
      return;
   if (onepage)
      canvas->cd(++ipad);
   hTrueXY->Draw();
...
{
   return Eff(x, xlo, xhi, effxlo, effxhi) && Eff(y, ylo, yhi, effylo, effyhi) && Eff(z, zlo, zhi, effzlo, effzhi);
}
...
RooUnfoldTestHarness3D::RooUnfoldTestHarness3D(const char *name) : RooUnfoldTestHarness2D(name)
{
   Reset();
   SetDefaults();
}
...
RooUnfoldTestHarness3D::RooUnfoldTestHarness3D(const char *name, const char *args) : RooUnfoldTestHarness2D(name)
{
   Reset();
...
void RooUnfoldTestHarness3D::Reset()
{
   ...
   hTrainZ = hTrainTrueZ = hTrueZ = hTrainFakeZ = hFakeZ = hMeasZ = hUnfoldZ = hPDFz = hTestPDFz = hPullsZ = 0;
   hTrueXY = hTrueYZ = hTrueXZ = hMeasXY = hMeasYZ = hMeasXZ = hUnfoldXY = hUnfoldYZ = hUnfoldXZ = 0;
   dim = 3;
}
...
Int_t RooUnfoldTestHarness3D::CheckParms()
{
   ...
   if (nmz <= 0) {
      cerr << "Error: nmz (" << nmz << ") <= 0" << endl;
      error = 2;
   }
   if (ntz <= 0) {
      cerr << "Error: ntz (" << ntz << ") <= 0" << endl;
      error = 2;
   }
   if (zlo >= zhi) {
      cerr << "Error: zlo (" << zlo << ") >= zhi(" << zhi << ")" << endl;
      error = 2;
   }
   if (ftrainz < 0) {
      cerr << "Error: ftrainz (" << ftrainz << ") < 0" << endl;
      error = 2;
   }
   if (ftestz < 0) {
      cerr << "Error: ftestz (" << ftestz << ") < 0" << endl;
      error = 2;
   }
   return error;
}
...
TH1D *
RooUnfoldTestHarness3D::Projection1D(const TH1 *h, Option_t *xyz, const char *name, const char *title, Option_t *opt)
{
...
TH2D *
RooUnfoldTestHarness3D::Projection2D(const TH1 *h, Option_t *xyz, const char *name, const char *title, Option_t *opt)
{
   const TH3 *h3 = dynamic_cast<const TH3 *>(h);
   TString opts(xyz);
   opts += opt;
