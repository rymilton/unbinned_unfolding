...
ArgVars &ArgVars::Add(ArgVar *arg)
{
   ...
   while ((argx = dynamic_cast<const ArgVar *>(next()))) {
      ...
      if (pvar == argx->ivar || pvar == argx->fvar || pvar == argx->svar) {
         std::cerr << "ArgVars::Add(" << arg->GetName() << ") refers to the same variable as '" << argx->GetName()
                   << "'" << std::endl;
         break;
      }
   }
...
{
   std::cout << "Usage: " << prog << " [PARAMETER=VALUE [PARAMETER=VALUE ...]]" << std::endl
             << std::endl
...
void ArgVars::ArgHelp(std::ostream &o) const
{
   ...
   while ((arg = dynamic_cast<const ArgVar *>(next()))) {
      ...
      o.width(10);
      o.setf(std::ios_base::left, std::ios_base::adjustfield);
      o << arg->GetName() << ' ';
      o.flags(f);
      o.width(7);
      if (!arg->setdef)
...
{
   if (s)
      return (strncmp(p, opt, s - p) == 0);
   else
      return (strcmp(p, opt) == 0);
}
...
void ArgVars::SetDefaults() const
{
   ...
   while ((arg = dynamic_cast<const ArgVar *>(next()))) {
      if (!arg->setdef)
         continue;
      if (arg->svar)
         arg->svar->Replace(0, arg->svar->Length(), arg->sdef); // use Replace to work round CINT bug #75875
      else if (arg->ivar)
         *(arg->ivar) = arg->idef;
      else
         *(arg->fvar) = arg->fdef;
...
Int_t ArgVars::SetArgs(int argc, const char *const *argv, bool split) const
{
   ...
   for (int i = 1; i < argc; i++) {
      ...
      while (p) {
         ...
         if (CmpOpt(p, "-h", s) || CmpOpt(p, "--help", s)) {
            help = true;
            if (s) {
               p = s + 1;
               continue;
         ...
         if (d) {
            ...
            if (arg) {
               ...
               if (arg->svar) {
                  ...
                  *(arg->ivar) = strtol(d, (char **)&nxt, 10);
               else
                  *(arg->fvar) = strtod(d, (char **)&nxt);
               if (split && nxt && *nxt == ' ')
                  q = nxt + 1;
