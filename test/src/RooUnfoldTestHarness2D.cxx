...
void RooUnfoldTestHarness2D::TrainResults()
{
   ...
   if (onepage && (onepage < 7 || ipad >= onepage))
      return;
   if (onepage)
      canvas->cd(++ipad);
   hTrainTrueX->Draw();
   ...
   if (overflow == 2) {
      ...
      for (Int_t ix = 2; ix <= ntx - 1; ix++) {
         for (Int_t iy = 2; iy <= nty - 1; iy++) {
            if (hUnfold->GetBinContent(ix, iy) != 0.0 && hUnfold->GetBinError(ix, iy) > 0.0 &&
                (hTrue->GetBinContent(ix, iy) != 0.0 || (doerror && hTrue->GetBinError(ix, iy) > 0.0))) {
               Double_t pull =
                  (hUnfold->GetBinContent(ix, iy) - hTrue->GetBinContent(ix, iy)) / hUnfold->GetBinError(ix, iy);
               if (!kilroy || pull < minpull)
                  minpull = pull;
               if (!kilroy || pull > maxpull)
                  maxpull = pull;
               kilroy++;
            }
         }
...
{
   return Eff(x, xlo, xhi, effxlo, effxhi) && Eff(y, ylo, yhi, effylo, effyhi);
}
...
RooUnfoldTestHarness2D::RooUnfoldTestHarness2D(const char *name) : RooUnfoldTestHarness(name)
{
   Reset();
   SetDefaults();
}
...
RooUnfoldTestHarness2D::RooUnfoldTestHarness2D(const char *name, const char *args) : RooUnfoldTestHarness(name)
{
   Reset();
...
void RooUnfoldTestHarness2D::Reset()
{
   ...
   hTrainX = hTrainTrueX = hTrueX = hTrainFakeX = hFakeX = hMeasX = hUnfoldX = hPullsX = hTrainY = hTrainTrueY =
      hTrueY = hTrainFakeY = hFakeY = hMeasY = hUnfoldY = hPullsY = hPDFy = hTestPDFy = 0;
   dim = 2;
}
...
Int_t RooUnfoldTestHarness2D::CheckParms()
{
   ...
   if (nmy <= 0) {
      cerr << "Error: nmy (" << nmy << ") <= 0" << endl;
      error = 2;
   }
   if (nty <= 0) {
      cerr << "Error: nty (" << nty << ") <= 0" << endl;
      error = 2;
   }
   if (ylo >= yhi) {
      cerr << "Error: ylo (" << ylo << ") >= yhi(" << yhi << ")" << endl;
      error = 2;
   }
   if (ftrainy < 0) {
      cerr << "Error: ftrainy (" << ftrainy << ") < 0" << endl;
      error = 2;
   }
   if (ftesty < 0) {
      cerr << "Error: ftesty (" << ftesty << ") < 0" << endl;
      error = 2;
   }
   if (dofit) {
      cerr << "Warning: parametric fit test only implemented for 1D" << endl;
      dofit = 0;
   }
   return error;
}
