...
def prepare(smear, write):
    ...
    for i in range(nevents):
        ...
        if x != None:
            sigbkg_reco_alt.Fill(x)

    for i in range(nevents):
        x = ROOT.gRandom.Uniform(-10, 10.0)
        bkg_reco.Fill(x)
        xd = ROOT.gRandom.Uniform(-10, 10.0)
        sigbkg_reco.Fill(xd)
        sigbkg_reco_alt.Fill(xd)

    histograms = {
        "sig_theory": sig_theory,
...
def makeFinalWorkspace(method, mainws, auxws, regparm):
    ...
    unfold_response = ROOT.RooFitUnfoldResponse(
        "unfold_response",
        "unfold_response",
        sig_response,
        sig_truth,
...
def makePlots(outdir, ws, obsname, hist_truth, hist_reco, label):
    ...
    unfoldpdf.plotOn(
        ...
        ROOT.RooFit.Name("unfold_graph_sys"),
        ROOT.RooFit.VisualizeError(prefit_sys),
        ROOT.RooFit.Normalization(nexp, ROOT.RooAbsReal.NumEvent),
        ROOT.RooFit.NormRange("full"),
    )
    unfoldpdf.plotOn(
        plot_truth,
        ROOT.RooFit.LineColor(ROOT.kBlack),
        ROOT.RooFit.Name("unfold_graph"),
        ROOT.RooFit.MarkerColor(ROOT.kBlack),
        ROOT.RooFit.MarkerSize(1),
        ROOT.RooFit.MarkerStyle(20),
        ROOT.RooFit.DrawOption("P"),
        ROOT.RooFit.VisualizeError(prefit_all),
        ROOT.RooFit.Normalization(nexp, ROOT.RooAbsReal.NumEvent),
        ROOT.RooFit.NormRange("full"),
    )
    sig_truth.plotOn(
        ...
        ROOT.RooFit.FillStyle(3345),
        ROOT.RooFit.Name("sig_truth_graph"),
        ROOT.RooFit.Normalization(nexp, ROOT.RooAbsReal.NumEvent),
        ROOT.RooFit.NormRange("full"),
    )
    ...
    data_minus_bkg_reco.plotOn(
        plot_reco,
        ROOT.RooFit.Name("data_minus_bkg_reco_graph"),
    ...
    sig_reco.plotOn(
        plot_reco,
        ROOT.RooFit.LineColor(ROOT.kRed),
        ROOT.RooFit.Name("sig_reco_graph"),
        ROOT.RooFit.Normalization(hist_reco.Integral(), ROOT.RooAbsReal.NumEvent),
        ROOT.RooFit.NormRange("full"),
    )
...
def makePlots_HistFactory(ws):
    ...
    sigbkg_reco_hist = unfolding.response().makeHistSum(sig_reco_hist.func(), bkg_reco, 1.0, 1.0)
...
def main(args):
    ...
    if args.mode == "HistFactory":
        ...
        spec = ROOT.RooUnfoldSpec(
            "unfold",
            "unfold",
            histograms["sig_truth"],
            "obs_truth",
            histograms["sig_reco"],
            "obs_reco",
            histograms["sig_response"],
            histograms["bkg_reco"],
            histograms["sigbkg_reco"],
            False,
            0.0005,
            False,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--smear", default=0.2, type=float)
