...
def axes(ratio_plot=True, figsize=(4,4), gridspec_update=None,
    ...
    if ratio_plot:
        axes[1].set_ylabel(ylabel_ratio, fontsize=12)
...
def calc_hist(vals, bins=10, weights=None, hist_range = None, density=True, efficiency=None):
    ...
    if hist_range is not None:
        hist, bins = np.histogram(vals, bins=bins, range=hist_range, weights=weights)
    else:
        hist, bins = np.histogram(vals, bins=bins, weights=weights)
    if efficiency is not None:
        hist = np.divide(hist, efficiency, out=np.zeros_like(hist, dtype=np.float32),
                                    where=((efficiency!=0)&(~np.isnan(efficiency))&(~np.isinf(efficiency))))
    ...
    if density:
        density_int = np.sum(hist) * (bins[1] - bins[0])
        hist /= density_int
        errs /= density_int
...
def save(fig, name, add_watermark=False, **kwargs):
