...
public:
   ...
   public:
      ...
      SVDUnfold(const TVectorD &bdat, const TMatrixD &Bcov, const TVectorD &bini, const TVectorD &xini,
                const TMatrixD &Mdet, const TMatrixD &MdetE);
      SVDUnfold(const Hist *bdat, const TMatrixD &Bcov, const Hist *bini, const Hist *xini, const Hist2D *Adet);
      SVDUnfold(const Hist *bdat, const TMatrixD &Bcov, const Hist *bini, const Hist *xini, const TMatrixD &Mdet,
                const TMatrixD &MdetE);
      ...
      // "ntoys"  - number of pseudo experiments used for the propagation
      // "seed"   - seed for pseudo experiments
      // "uncmat" - matrix containing the uncertainty on the detector matrix elements if different from purely
      // statistical without any weights
...
public:
   ...
   RooUnfoldSvdT(const RooUnfoldResponseT<Hist, Hist2D> *res, const Hist *meas, Int_t kreg = 0, const char *name = 0,
                 const char *title = 0);
