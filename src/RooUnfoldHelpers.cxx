...
TMatrixD *squareMatrix(const TMatrixD &matrix, int nbins)
{
   ...
   if (nbins < 0)
      nbins = std::max(matrix.GetNrows(), matrix.GetNcols());
...
void printTable(std::ostream &o, int dim, int ntxb, int ntyb, const TVectorD &vTrainTrue, const TVectorD &vTrain,
                ...
                const TVectorD &eTrue, const TVectorD &eReco, double chi_squ, bool overflow)
{
   ...
   o << "===============================================================================" << xwid << std::endl;
   o << std::setw(iwid) << "" << std::setw(9) << "Train" << std::setw(9) << "Train" << std::setw(9) << "Test"
     << std::setw(9) << "Test" << std::setw(9) << "Unfolded";
   if (withError)
      o << std::setw(10) << "Error on" << std::setw(9) << "Diff" << std::setw(9) << "Pull" << std::endl;
   else
      o << std::setw(9) << "Diff" << std::endl;
   o << std::setw(iwid) << "Bin" << std::setw(9) << "Truth" << std::setw(9) << "Measured" << std::setw(9) << "Truth"
     << std::setw(9) << "Input" << std::setw(9) << "Output";
   if (withError)
   ...
   for (Int_t i = 0; i < maxbin; i++) {
      ...
      if (dim == 2 || dim == 3) {
         ...
         if (dim == 3)
            o << ',' << std::setw(iw) << ((it - ix) / ntxb - iy) / ntyb + first;
      } else {
      ...
      if (i < _nt) {
         ...
         if (withError)
            o << ' ' << std::setw(9) << yerr;
         if (vTrue.GetNrows() > 0 && (y != 0.0 || (withError && yerr > 0.0)) &&
   ...
   o << "===============================================================================" << xwid << std::endl
     << std::setw(iwid) << "" << std::fixed << std::setprecision(0) << ' ' << std::setw(8) << true_train_tot << ' '
   ...
   if (vTrue.GetNrows() > 0)
      ...
      o << std::setw(9) << ' ';
   Double_t toterr = 0.0;
   if (meas_test_tot > 0.0 && meas_train_tot > 0.0)
      toterr = sqrt(meas_test_tot) * true_train_tot / meas_train_tot;
   o << ' ' << std::setw(8) << meas_test_tot << std::setprecision(1) << ' ' << std::setw(8) << unf_tot;
   if (withError)
      o << ' ' << std::setw(9) << toterr;
   ...
   if (vTrue.GetNrows() > 0) {
      ...
      if (chi_squ <= 0.0)
         std::cerr << "Warning: Invalid Chi^2 Value" << std::endl;
   }
}
                ...
                const TVectorD &vReco)
{
   ...
   printTable(o, 1, vTrainTrue.GetNrows(), 0, vTrainTrue, vTrain, vTrue, vMeas, vReco, kNoError, eTrue, eReco, -999.0,
              false);
}
...
void assert_compat(const TMatrixD &a, const TMatrixD &b)
{
   if (a.GetNcols() != b.GetNcols() || a.GetNrows() != b.GetNrows())
      throw std::runtime_error(
         TString::Format("matrix operation impossible due to size mismatch: arguments are of size %d/%d and %d/%d",
...
void assert_compat(const TVectorD &a, const TVectorD &b)
{
   if (a.GetNrows() != b.GetNrows())
      throw std::runtime_error(
         TString::Format("vector operation impossible due to size mismatch: arguments are of size %d and %d",
