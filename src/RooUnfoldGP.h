...
public:
   ...
   RooUnfoldGPT(const RooUnfoldResponseT<Hist, Hist2D> *res, const Hist *meas, Int_t kernel = 1, const char *name = 0,
                const char *title = 0);
...
private:
   ...
   void
   GibbsKernel(const std::vector<Double_t> &params, const TVectorD &x, const TVectorD &xprime, TMatrixD &matrix) const;
   void
   RadialKernel(const std::vector<Double_t> &params, const TVectorD &x, const TVectorD &xprime, TMatrixD &matrix) const;
