...
public:
   static RooUnfoldT<Hist, Hist2D> *New(RooUnfolding::Algorithm alg, const RooUnfoldResponseT<Hist, Hist2D> *res,
                                        const Hist *meas, Double_t regparm = -1e30, const char *name = 0,
                                        const char *title = 0);
   ...
   RooUnfoldT(const RooUnfoldResponseT<Hist, Hist2D> *res, const Hist *meas, const char *name = 0,
              const char *title = 0);
   ...
   DEPRECATED("Hreco is considered ambigious and should no longer be used, please use Hunfold instead")
   virtual Hist *Hreco() { return Hunfold(RooUnfolding::kErrors); }
   ...
   const TVectorD
   Pull(Int_t binN = 1) const; // Pull results of each unfolding instance as a TVectorD for a specified bin Number
   const TVectorD
   Ebias(RooUnfolding::BiasError E_type = RooUnfolding::kBiasSDM) const; // Bias distribution errors as a TVectorD
   ...
   virtual void
   PrintTable(const Hist *hTrue = 0, RooUnfolding::ErrorTreatment withError = RooUnfolding::kDefault) const;
   virtual void PrintTable(std::ostream &o, const Hist *hTrue = 0,
                           RooUnfolding::ErrorTreatment withError = RooUnfolding::kDefault) const;
   ...
   virtual void CalculatePull(Int_t ntoys = 50, const Hist *hTrue = 0)
      const; // Calculate pulls for each bin and each unfolding instance and calculate the average pull for each bin
   ...
   void
   RunRooFitToys(int ntoys, std::vector<TVectorD> &vx, std::vector<TVectorD> &vxe, std::vector<double> &chi2) const;
...
protected:
   ...
   RooUnfolding::SystematicsTreatment
      _dosys; // include systematic errors from response matrix? use _dosys=2 to exclude measurement errors
...
public:
   ClassDefOverride(RooUnfoldT, 2) // Unfolding base class: implementations in RooUnfoldBayes, RooUnfoldSvd,
                                   // RooUnfoldBinByBin, RooUnfoldTUnfold, RooUnfoldInvert, RooUnfoldIds
};
