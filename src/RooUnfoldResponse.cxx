...
<p> Can also take a variety of parameters as inputs. This includes maximum and minimum values, distributions and
vectors/matrices of values. </p> <p> This class does the numerical modifications needed to allow unfolding techniques to
work in the unfolding routines used in RooUnfold. </p>
...
public:
   RooUnfoldFoldingFunction(const RooUnfoldResponseT<Hist, Hist2D> *res, TF1 *func, Double_t eps = 1e-12,
                            bool verbose = false)
      : _res(res), _func(func), _eps(eps), _verbose(verbose), _fvals(_res->Vmeasured().GetNrows())
   {
      _ndim = dynamic_cast<TF3 *>(_func) ? 3 : dynamic_cast<TF2 *>(_func) ? 2 : 1;
      if (_ndim >= 2 && eps == 1e-12)
         eps = 0.000001;
      FVals();
   }
   ...
   double operator()(double *x, double *p) const
   {
      ...
      Int_t bin;
      if (_ndim == 1)
         bin = RooUnfolding::findBin(mes, x[0]);
      else if (_ndim == 2)
         bin = RooUnfolding::findBin(mes, x[0], x[1]);
      else
         bin = RooUnfolding::findBin(mes, x[0], x[1], x[2]);
      if (bin < 0 || bin >= _res->Vmeasured().GetNrows())
         return 0.0;
      for (Int_t i = 0, n = _func->GetNpar(); i < n; i++) {
         if (p[i] == _func->GetParameter(i))
            continue;
...
private:
   void FVals() const
   {
      ...
      if (_verbose) {
         cout << "p=";
         for (int i = 0, n = _func->GetNpar(); i < n; i++)
            cout << _func->GetParameter(i) << ",";
         cout << " f=";
      }
      _fvals.Zero();
      for (Int_t i = 0, n = _res->Vtruth().GetNrows(); i < n; i++) {
         Int_t jx, jy, jz;
         binXYZ(tru, i, jx, jy, jz);
         Double_t fv;
         if (_eps <= 0.0) {
            if (_ndim >= 2)
               fv = _func->Eval(binCenter(tru, jx, RooUnfolding::X), binCenter(tru, jy, RooUnfolding::Y),
                                binCenter(tru, jz, RooUnfolding::Z));
            else
               fv = _func->Eval(binCenter(tru, jx, RooUnfolding::X));
         } else {
            if (_ndim == 1) {
               Double_t tw = binWidth(tru, jx, RooUnfolding::X), tlo = binLowEdge(tru, jx, RooUnfolding::X),
                        thi = tlo + tw;
#if ROOT_VERSION_CODE >= ROOT_VERSION(6, 0, 0)
               fv = _func->Integral(tlo, thi, _eps) / tw;
            } else {
               Double_t tlo[3] = {binLowEdge(tru, jx, RooUnfolding::X), binLowEdge(tru, jy, RooUnfolding::Y),
                                  binLowEdge(tru, jz, RooUnfolding::Z)};
               Double_t thi[3] = {binHighEdge(tru, jx, RooUnfolding::X), binHighEdge(tru, jy, RooUnfolding::Y),
                                  binHighEdge(tru, jz, RooUnfolding::Z)};
               Double_t relerr = 0.0;
               fv = _func->IntegralMultiple(_ndim, tlo, thi, _eps, relerr);
               fv /= binWidth(tru, jx, RooUnfolding::X) * binWidth(tru, jy, RooUnfolding::Y);
               if (_ndim >= 3)
                  fv /= binWidth(tru, jz, RooUnfolding::Z);
#else
               ...
               fv = _func->Integral(binLowEdge(tru, jx, RooUnfolding::X), binHighEdge(tru, jx, RooUnfolding::X),
                                    binLowEdge(tru, jy, RooUnfolding::Y), binHighEdge(tru, jy, RooUnfolding::Y), _eps);
               fv /= binWidth(tru, jx, RooUnfolding::X) * binWidth(tru, jy, RooUnfolding::Y);
            } else {
               fv = _func->Integral(binLowEdge(tru, jx, RooUnfolding::X), binHighEdge(tru, jx, RooUnfolding::X),
                                    binLowEdge(tru, jy, RooUnfolding::Y), binHighEdge(tru, jy, RooUnfolding::Y),
                                    binLowEdge(tru, jz, RooUnfolding::Z), binHighEdge(tru, jz, RooUnfolding::Z), _eps);
               fv /= binWidth(tru, jx, RooUnfolding::X) * binWidth(tru, jy, RooUnfolding::Y) *
                     binWidth(tru, jz, RooUnfolding::Z);
#endif
            }
         }
         if (_verbose)
            cout << " " << fv;
         for (Int_t ibin = 0, m = _res->Vmeasured().GetNrows(); ibin < m; ibin++) {
...
template <class Hist, class Hist2D>
void RooUnfoldResponseT<Hist, Hist2D>::Init()
{
   _tru = _mes = _fak = 0;
   _res = 0;
   this->ClearCache();
...
template <class Hist, class Hist2D>
void RooUnfoldResponseT<Hist, Hist2D>::ClearCache() const
{
   if (Cached()) {
      _cache = Cache();
      _cached = false;
...
template <class Hist, class Hist2D>
Hist2D *RooUnfoldResponseT<Hist, Hist2D>::HresponseNoOverflow() const
{
...
template <class Hist, class Hist2D>
void RooUnfoldResponseT<Hist, Hist2D>::Print(Option_t * /* option */) const
{
...
template <class Hist, class Hist2D>
bool RooUnfoldResponseT<Hist, Hist2D>::Cached() const
{
   return _cached;
}
...
template <class Hist, class Hist2D>
typename RooUnfoldResponseT<Hist, Hist2D>::Cache &
RooUnfoldResponseT<Hist, Hist2D>::Cache::operator=(const RooUnfoldResponseT<Hist, Hist2D>::Cache &other)
{
   //! assignment operator
   if (this->_vMes) {
      delete this->_vMes;
   }
   this->_vMes = other._vMes;
   if (this->_eMes) {
      delete this->_eMes;
   }
   this->_eMes = other._eMes;
   if (this->_vFak) {
      delete this->_vFak;
   }
   this->_vFak = other._vFak;
   if (this->_vTru) {
      delete this->_vTru;
   }
   this->_vTru = other._vTru;
   if (this->_eTru) {
      delete this->_eTru;
   }
   this->_eTru = other._eTru;
   if (this->_mRes) {
      delete this->_mRes;
   }
   this->_mRes = other._mRes;
   if (this->_eRes) {
      delete this->_eRes;
   }
   this->_eRes = other._eRes;
   if (this->_mResNorm) {
      delete this->_mResNorm;
   }
   this->_mResNorm = other._mResNorm;
   if (this->_eResNorm) {
      delete this->_eResNorm;
   }
   this->_eResNorm = other._eResNorm;
   if (this->_vEff) {
      delete this->_vEff;
   }
   this->_vEff = other._vEff;
   if (this->_vPur) {
      delete this->_vPur;
   }
   this->_vPur = other._vPur;
   return *this;
}
...
template <class Hist, class Hist2D>
Hist *RooUnfoldResponseT<Hist, Hist2D>::ApplyToTruth(const Hist *truth, const char *name) const
{
   //! Apply the response matrix to the truth
   //! Errors not set, since we assume original truth has no errors
   if (!Htruth())
      return 0; // Needed for checking binning if nothing else
   ...
   if (truth) {
      if (nBins(truth, RooUnfolding::X) != nBins(_tru, RooUnfolding::X) ||
         ...
         cerr << "Warning: RooUnfoldResponseT<Hist,Hist2D>::ApplyToTruth truth histogram is a different size ("
              << (nBins(truth, RooUnfolding::X) * nBins(truth, RooUnfolding::Y) * nBins(truth, RooUnfolding::Z))
              << " bins) or shape from response matrix truth ("
              << (nBins(_tru, RooUnfolding::X) * nBins(_tru, RooUnfolding::Y) * nBins(_tru, RooUnfolding::Z))
              << " bins)" << endl;
...
template <class Hist, class Hist2D>
TF1 *RooUnfoldResponseT<Hist, Hist2D>::MakeFoldingFunction(TF1 *func, Double_t eps, Bool_t verbose) const
{
   ...
   if (TF3 *func3 = dynamic_cast<TF3 *>(func))
      f = new TF3(name, ROOT::Math::ParamFunctor(ff), func3->GetXmin(), func3->GetXmax(), func3->GetYmin(),
                  func3->GetYmax(), func3->GetZmin(), func3->GetZmax(), np);
   else if (TF2 *func2 = dynamic_cast<TF2 *>(func))
      f = new TF2(name, ROOT::Math::ParamFunctor(ff), func2->GetXmin(), func2->GetXmax(), func2->GetYmin(),
                  func2->GetYmax(), np);
   else
      f = new TF1(name, ROOT::Math::ParamFunctor(ff), func->GetXmin(), func->GetXmax(), np);
   int nm = Vmeasured().GetNrows();
   f->SetNpx(nm <= 2 ? 4 : nm == 3 ? 6 : nm); // TF1 requires Npx>=4
...
template <class Hist, class Hist2D>
void RooUnfoldResponseT<Hist, Hist2D>::RunToy() const
{
   this->ClearCache();
   Vmeasured();
   Vtruth();
...
template <class Hist, class Hist2D>
void RooUnfoldResponseT<Hist, Hist2D>::SetNameTitleDefault(const char *defname, const char *deftitle)
{
...
template <class Hist, class Hist2D>
void RooUnfoldResponseT<Hist, Hist2D>::Streamer(TBuffer &R__b)
{
   if (R__b.IsReading()) {
...
template <>
void RooUnfoldResponseT<TH1, TH2>::Streamer(TBuffer &R__b)
{
   if (R__b.IsReading()) {
...
template <class Hist, class Hist2D>
RooUnfoldResponseT<Hist, Hist2D>::RooUnfoldResponseT() : TNamed()
{
...
template <class Hist, class Hist2D>
RooUnfoldResponseT<Hist, Hist2D>::RooUnfoldResponseT(const char *name, const char *title) : TNamed(name, title)
{
...
template <class Hist, class Hist2D>
RooUnfoldResponseT<Hist, Hist2D>::RooUnfoldResponseT(const TString &name, const TString &title) : TNamed(name, title)
{
   //! default named constructor. Use setup() to set values.
   this->Init();
}
...
template <class Hist, class Hist2D>
RooUnfoldResponseT<Hist, Hist2D>::RooUnfoldResponseT(const char *name, const char *title, Hist2D *response, Hist *truth,
                                                     Hist *reco, bool overflow, bool density)
   : TNamed(name, title), _mes(reco), _tru(truth), _res(response), _overflow(overflow), _density(density)
{
...
Int_t RooUnfoldResponseT<Hist, Hist2D>::GetDimensionMeasured() const
{
   ...
   return RooUnfolding::dim(this->_mes);
}
...
Int_t RooUnfoldResponseT<Hist, Hist2D>::GetDimensionTruth() const
{
   ...
   return RooUnfolding::dim(this->_tru);
}
...
TVectorD RooUnfoldResponseT<Hist, Hist2D>::Vfolded(const TVectorD &truth) const
{
   ...
   if (truth.GetNrows() != res.GetNcols())
      throw std::runtime_error("Error in RooUnfoldResponseT::Vfolded: invalid dimensionality in given truth vector!");
   auto prod = res * truth;
   return prod;
}
...
const TVectorD &RooUnfoldResponseT<Hist, Hist2D>::Vmeasured() const
{
   ...
   if (!_cache._vMes)
      _cached = (_cache._vMes = new TVectorD(h2v(_mes, _overflow, _density)));
   return *_cache._vMes;
}
...
const TVectorD &RooUnfoldResponseT<Hist, Hist2D>::Vfakes() const
{
   ...
   if (!_fak) {
      _cached = (_cache._vFak = new TVectorD(this->Vmeasured().GetNrows()));
   } else {
      if (!_cache._vFak)
         _cached = (_cache._vFak = new TVectorD(h2v(_fak, _overflow, _density)));
   }
   return *_cache._vFak;
}
...
const TVectorD &RooUnfoldResponseT<Hist, Hist2D>::Emeasured() const
{
   ...
   if (!_cache._eMes)
      _cached = (_cache._eMes = new TVectorD(h2ve(_mes, _overflow, _density)));
   return *_cache._eMes;
}
...
const TVectorD &RooUnfoldResponseT<Hist, Hist2D>::Vtruth() const
{
   ...
   if (!_cache._vTru)
      _cached = (_cache._vTru = new TVectorD(h2v(_tru, _overflow, _density)));
   return *_cache._vTru;
}
...
const TVectorD &RooUnfoldResponseT<Hist, Hist2D>::Etruth() const
{
   ...
   if (!_cache._eTru)
      _cached = (_cache._eTru = new TVectorD(h2ve(_tru, _overflow, _density)));
   return *_cache._eTru;
}
...
const TMatrixD &RooUnfoldResponseT<Hist, Hist2D>::Mresponse(bool norm) const
{
   ...
   if (norm) {
      if (!_cache._mResNorm)
         _cached = (_cache._mResNorm = new TMatrixD(h2mNorm(_res, _tru, _overflow, _density)));
      return *_cache._mResNorm;
   } else {
      if (!_cache._mRes)
         _cached = (_cache._mRes = new TMatrixD(h2m(_res, _overflow, _density)));
      return *_cache._mRes;
   }
}
...
const TMatrixD &RooUnfoldResponseT<Hist, Hist2D>::Eresponse(bool norm) const
{
   ...
   if (norm) {
      if (!_cache._eResNorm)
         _cached = (_cache._eResNorm = new TMatrixD(h2meNorm(_res, _tru, _overflow, _density)));
      return *_cache._eResNorm;
   } else {
      if (!_cache._eRes)
         _cached = (_cache._eRes = new TMatrixD(h2me(_res, _overflow, _density)));
      return *_cache._eRes;
   }
}
...
template <class Hist, class Hist2D>
void projectY(Hist2D *_res, Hist *_tru, bool overflow);
template <class Hist, class Hist2D>
void projectX(Hist2D *_res, Hist *_mes, bool overflow);
template <class Hist, class Hist2D>
void subtractProjectX(Hist2D *_res, Hist *_mes, Hist *_fak, bool overflow);

template <>
void projectY<TH1>(TH2 *_res, TH1 *_tru, bool overflow)
{
   ...
   for (Int_t j = 1 - overflow; j < _res->GetNbinsY() + 1 + overflow; j++) {
      ...
      for (Int_t i = 0; i < _res->GetNbinsX() + 2; i++) {
         ...
         if (s)
            wtru += pow(_res->GetBinError(i, j), 2);
      }
      Int_t b = bin<TH1>(_tru, j, overflow);
      _tru->SetBinContent(b, ntru);
      if (s)
         _tru->SetBinError(b, sqrt(wtru));
   }
}
template <>
void projectX<TH1>(TH2 *_res, TH1 *_mes, bool overflow)
{
   Int_t s = _res->GetSumw2N();
   for (Int_t i = 1 - overflow; i < _res->GetNbinsX() + 1 + overflow; i++) {
      ...
      if (s)
         _mes->SetBinError(b, sqrt(wmes));
   }
}
template <>
void subtractProjectX<TH1>(TH2 *_res, TH1 *_mes, TH1 *_fak, bool overflow)
{
   Int_t s = _res->GetSumw2N();
...
template <class Hist>
int fill(Hist *hist, double x, double w);
template <class Hist>
int fill(Hist *hist, double x, double y, double w);
template <class Hist>
int fill(Hist *hist, double x, double y, double z, double w);
template <>
int fill<TH1>(TH1 *hist, double x, double w)
{
   return hist->Fill(x, w);
}
template <>
int fill<TH1>(TH1 *hist, double x, double y, double w)
{
   return ((TH2 *)hist)->Fill(x, y, w);
}
template <>
int fill<TH1>(TH1 *hist, double x, double y, double z, double w)
{
   return ((TH3 *)hist)->Fill(x, y, z, w);
}
template <>
int fill<TH2>(TH2 *hist, double x, double y, double w)
{
...
RooUnfoldResponse::RooUnfoldResponse(const RooUnfoldResponse &rhs) : RooUnfoldResponseT(rhs.GetName(), rhs.GetTitle())
{
   //! RooUnfoldResponseT<class Hist, class Hist2D> copy constructor
   Setup(rhs);
}

RooUnfoldResponse &RooUnfoldResponse::operator=(const RooUnfoldResponse &rhs)
{
...
RooUnfoldResponse &RooUnfoldResponse::Setup(const RooUnfoldResponse &rhs)
{
...
RooUnfoldResponse &RooUnfoldResponse::Reset()
{
...
RooUnfoldResponse::RooUnfoldResponse(Int_t nb, Double_t xlo, Double_t xhi, const char *name, const char *title)
...
RooUnfoldResponse::RooUnfoldResponse(const TH1 *measured, const TH1 *truth, const TH2 *response, const char *name,
                                     const char *title, bool overflow)
   : RooUnfoldResponseT(name, title)
{
...
RooUnfoldResponse::RooUnfoldResponse(const TH1 *measured, const TH1 *truth, const char *name, const char *title,
                                     bool overflow)
   : RooUnfoldResponseT(name, title)
{
...
RooUnfoldResponse &RooUnfoldResponse::Setup(Int_t nm, Double_t mlo, Double_t mhi, Int_t nt, Double_t tlo, Double_t thi)
{
...
RooUnfoldResponse &RooUnfoldResponse::Setup(const TH1 *measured, const TH1 *truth)
{
   ...
   auto mes_vars = RooUnfolding::vars(measured);
   auto tru_vars = RooUnfolding::vars(truth);
...
RooUnfoldResponse &RooUnfoldResponse::Setup(const TH1 *measured, const TH1 *truth, const TH2 *response)
{
...
Int_t RooUnfoldResponse::Fill(Double_t xr, Double_t xt, Double_t w)
{
   ...
   int retval = fill(_res, xr, xt, w);
   return retval;
}

Int_t RooUnfoldResponse::Fill(Double_t xr, Double_t yr, Double_t xt, Double_t yt, Double_t w)
{
   ...
   return fill(_res, binCenter(_res, RooUnfolding::findBin(_mes, xr, yr) + 1, RooUnfolding::X),
               binCenter(_res, RooUnfolding::findBin(_tru, xt, yt) + 1, RooUnfolding::Y), w);
}

Int_t RooUnfoldResponse::Fill(Double_t xr, Double_t yr, Double_t zr, Double_t xt, Double_t yt, Double_t zt, Double_t w)
{
   ...
   return fill(_res, binCenter(_res, RooUnfolding::findBin(_mes, xr, yr, zr) + 1, RooUnfolding::X),
               binCenter(_res, RooUnfolding::findBin(_tru, xt, yt, zt) + 1, RooUnfolding::Y), w);
}

Int_t RooUnfoldResponse::FindBin(const TH1 *h, Double_t x, Double_t y, Double_t z)
{
   ...
   Int_t binx = RooUnfolding::findBin(h, x, RooUnfolding::X) - 1;
   if (binx < 0)
      return -1;
   if (binx >= nx)
      return nx * ny * nz;
   Int_t biny = RooUnfolding::findBin(h, y, RooUnfolding::Y) - 1;
   if (biny < 0)
      return -1;
   if (biny >= ny)
      return nx * ny * nz;
   Int_t binz = RooUnfolding::findBin(h, z, RooUnfolding::Z) - 1;
   if (binz < 0)
      return -1;
   if (binz >= nz)
      return nx * ny * nz;
...
Int_t RooUnfoldResponse::Miss1D(Double_t xt, Double_t w)
{
...
Int_t RooUnfoldResponse::Miss2D(Double_t xt, Double_t yt, Double_t w)
{
...
Int_t RooUnfoldResponse::Miss(Double_t xt, Double_t yt, Double_t zt, Double_t w)
{
...
Int_t RooUnfoldResponse::Fake1D(Double_t xr, Double_t w)
{
...
Int_t RooUnfoldResponse::Fake2D(Double_t xr, Double_t yr, Double_t w)
{
...
Int_t RooUnfoldResponse::Fake(Double_t xr, Double_t yr, Double_t zr, Double_t w)
{
...
{
   template <class T>
   void makeList(const RooAbsCollection *c, std::vector<T *> &vars, TPRegexp *re = 0)
   {
      for (auto *item : *c) {
   ...
   template <class T>
   std::vector<T *> makeList(const RooAbsCollection *c)
   {
...
RooFitUnfoldResponse::RooFitUnfoldResponse(
   const RooUnfoldResponseT<RooUnfolding::RooFitHist, RooUnfolding::RooFitHist> *res)
   : RooUnfoldResponseT(res->GetName(), res->GetTitle(), static_cast<const RooFitUnfoldResponse *>(res)->_res->clone(),
                        static_cast<const RooFitUnfoldResponse *>(res)->_tru->clone(),
                        static_cast<const RooFitUnfoldResponse *>(res)->_mes->clone(), false,
                        static_cast<const RooFitUnfoldResponse *>(res)->_density)
{
}

RooFitUnfoldResponse::RooFitUnfoldResponse(const char *name, const char *title, RooUnfolding::RooFitHist *response,
                                           RooUnfolding::RooFitHist *truth, RooUnfolding::RooFitHist *reco,
                                           bool density)
...
RooFitUnfoldResponse::RooFitUnfoldResponse(const char * /*name*/, const char * /*title*/, RooAbsReal *response,
                                           RooAbsReal *truth, RooAbsReal *reco, RooAbsReal *fakes,
                                           RooRealVar *obs_truth, RooRealVar *obs_reco, bool density)
{
   if (!truth->dependsOn(*obs_truth))
      throw std::runtime_error("truth histogram does not depend on truth observable!");
   if (!reco->dependsOn(*obs_reco))
      throw std::runtime_error("reco histogram does not depend on reco observable!");
   if (!response->dependsOn(*obs_truth))
      throw std::runtime_error("response histogram does not depend on truth observable!");
   if (!response->dependsOn(*obs_reco))
      throw std::runtime_error("response histogram does not depend on reco observable!");
...
RooFitUnfoldResponse::RooFitUnfoldResponse(const char *name, const char *title, RooAbsReal *response, RooAbsReal *truth,
                                           RooAbsReal *reco, RooAbsReal *fakes, const RooAbsCollection *observables,
                                           bool density)
   : RooUnfoldResponseT(name, title)
{
   ...
   if (obs->getSize() != 2)
      throw std::runtime_error(TString::Format("unsupported dimensionality for response: %d", obs->getSize()).Data());
   if (obsset_reco->getSize() != 1)
      throw std::runtime_error(
         TString::Format("unsupported dimensionality for reco: %d", obsset_reco->getSize()).Data());
   if (obsset_truth->getSize() != 1)
      throw std::runtime_error(
         TString::Format("unsupported dimensionality for truth: %d", obsset_truth->getSize()).Data());
   ...
   this->_res = new RooFitHist(response, ::makeList<RooRealVar>(obs));
   this->_overflow = 0;
   this->_density = density;
}
...
RooHistFunc *RooFitUnfoldResponse::makeHistFunc(RooDataHist *dhist)
{
   ...
   for (size_t i = 0; i < this->_mes->dim(); ++i) {
      if (dhist->get()->find(*this->_mes->obs(i)))
         v.push_back(this->_mes->obs(i));
   }
   for (size_t i = 0; i < this->_tru->dim(); ++i) {
      if (dhist->get()->find(*this->_tru->obs(i)))
         v.push_back(this->_tru->obs(i));
   }
   if (v.size() == 0) {
      throw std::runtime_error(
         TString::Format(
            "unable to construct histogram from RooDataHist '%s', does not seem to depend on any known observable!",
            dhist->GetName())
            .Data());
   }
...
RooUnfolding::RooFitHist *RooFitUnfoldResponse::makeHist(RooAbsReal *object)
{
   ...
   std::vector<RooRealVar *> v;
   for (size_t i = 0; i < this->_mes->dim(); ++i) {
      if (object->dependsOn(*this->_mes->obs(i)))
         v.push_back(this->_mes->obs(i));
   }
   for (size_t i = 0; i < this->_tru->dim(); ++i) {
      if (object->dependsOn(*this->_tru->obs(i)))
         v.push_back(this->_tru->obs(i));
   }
   if (v.size() == 0) {
      throw std::runtime_error(
         TString::Format(
            "unable to construct histogram from RooAbsReal '%s', does not seem to depend on any known observable!",
            object->GetName())
            .Data());
   }
...
RooUnfolding::RooFitHist *RooFitUnfoldResponse::makeHist(RooDataHist *object)
{
   ...
   std::vector<RooRealVar *> v;
   TPRegexp obs("obs_.*");
   makeList<RooRealVar>(object->get(), v, &obs);
   return new RooUnfolding::RooFitHist(object, v);
}
