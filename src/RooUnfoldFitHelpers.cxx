...
struct RooPoissonMean {
   typedef RooRealProxy(RooPoisson::*type);
};
...
struct RooHistFuncObs {
   typedef RooArgSet(RooHistFunc::*type);
};
...
namespace {
template <class T>
T *findLeafServer(RooAbsArg *rr, const char *name)
{
   if (!rr)
      return NULL;
   for (RooAbsArg *server : rr->servers()) {
      if (!server)
         continue;
      if (strcmp(server->GetName(), name) == 0 &&
          (server->InheritsFrom(RooRealVar::Class()) || server->InheritsFrom(RooCategory::Class()))) {
...
template <class T>
RooArgList argList(const std::vector<T *> &vars)
{
   RooArgList retval;
   for (auto v : vars)
      retval.add(*v, true);
...
void Variable<RooUnfolding::RooFitHist>::print() const
{
   this->_var->Print("v");
}

bool Variable<RooUnfolding::RooFitHist>::irregular() const
{
   return !this->_var->getBinning().isUniform();
}

template <>
Variable<RooUnfolding::RooFitHist> var(const RooUnfolding::RooFitHist *h, Dimension d)
{
   return Variable<RooUnfolding::RooFitHist>(h->obs(d));
}

template <>
const char *name<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h)
{
   return h->name();
}
template <>
const char *title<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h)
{
   return h->title();
}

template <>
int findBin<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, double x, RooUnfolding::Dimension d)
{
   if (d > h->dim())
      throw std::runtime_error(
         TString::Format("unable to retrieve min for dimension %d for histogram %s with %d dimensions", d, name(h),
                         (int)(h->dim()))
            .Data());
   return ::binNumber(h->obs(d), x);
}
template <>
int findBin<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, Double_t x)
{
   if (h->dim() != 1)
      throw std::runtime_error(
         TString::Format("inacceptable call: findBin with one coordinate for %d dimensional histogram", (int)(h->dim()))
            .Data());
   return ::binNumber(h->obs(0), x);
}
template <>
int findBin<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, Double_t x, Double_t y)
{
   if (h->dim() != 2)
      throw std::runtime_error(
         TString::Format("inacceptable call: findBin with two coordinates for %d dimensional histogram",
                         (int)(h->dim()))
            .Data());
   return ::binNumber(h->obs(0), x) + ::nBins(h->obs(0)) * ::binNumber(h->obs(1), y);
}
template <>
int findBin<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, Double_t x, Double_t y, Double_t z)
{
   if (h->dim() != 3)
      throw std::runtime_error(
         TString::Format("inacceptable call: findBin with three coordinates for %d dimensional histogram",
                         (int)(h->dim()))
            .Data());
   return ::binNumber(h->obs(0), x) +
          ::nBins(h->obs(0)) * (::binNumber(h->obs(1), y) + ::nBins(h->obs(1)) * ::binNumber(h->obs(2), z));
}

template <>
double min<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, RooUnfolding::Dimension d)
{
   if (d > hist->dim())
      throw std::runtime_error(
         TString::Format("unable to retrieve min for dimension %d for histogram %s with %d dimensions", d, name(hist),
                         (int)(hist->dim()))
            .Data());
   return ::min(hist->obs(d));
}
template <>
double max<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, RooUnfolding::Dimension d)
{
   if (d > hist->dim())
      throw std::runtime_error(
         TString::Format("unable to retrieve max for dimension %d for histogram %s with %d dimensions", d, name(hist),
                         (int)(hist->dim()))
            .Data());
   return ::max(hist->obs(d));
}
template <>
int sumW2N<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist)
{
   return hist->weighted();
}
template <>
bool empty<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist * /*hist*/)
{
   return false;
}
template <>
int dim<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist)
{
   // get number of observables
   return hist->dim();
}
template <>
int nBins<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, bool /*overflow*/)
{
...
template <>
int nBins<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, RooUnfolding::Dimension d, bool /*overflow*/)
{
...
template <>
double binCenter<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, int i, RooUnfolding::Dimension d)
{
   if (hist->dim() <= d)
      return nan;
...
template <>
double binWidth<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, int i, RooUnfolding::Dimension d)
{
   if (hist->dim() <= d)
      return nan;
...
template <>
double binHighEdge<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, int i, RooUnfolding::Dimension d)
{
   if (hist->dim() <= d)
      return nan;
...
template <>
double binLowEdge<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, int i, RooUnfolding::Dimension d)
{
   if (hist->dim() <= d)
      return nan;
...
template <>
int bin<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist * /*hist*/, int i, Bool_t /*overflow*/)
{
   return i;
}
template <>
int bin<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, int i, int j, Bool_t /*overflow*/)
{
   int nx = nBins(hist, X);
   return j * nx + i;
}
template <>
int bin<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, int i, int j, int k, Bool_t /*overflow*/)
{
   int nx = nBins(hist, X);
   int ny = nBins(hist, Y);
...
template <>
void binXYZ<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, int binglobal, int &binx, int &biny,
                                      int &binz)
{
...
void setBin(const RooUnfolding::RooFitHist *h, int ibin)
{
   ...
   if (d > 2)
      setBin(h->obs(2), iz);
   if (d > 3)
      throw std::runtime_error(TString::Format("unable to handle histograms %s: %d dimensions!", h->name(), d).Data());
   if (h->bin() != ibin)
      throw std::runtime_error("internal binning error!");
}
} // namespace

namespace RooUnfolding {
template <>
const char *varname<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *hist, Dimension d)
{
   if (d > hist->dim())
      throw std::runtime_error(
         TString::Format("unable to retrieve name for dimension %d for histogram %s with %d dimensions", d, name(hist),
                         (int)(hist->dim()))
            .Data());
   return hist->obs(d)->GetName();
}
template <>
double binError<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, Int_t i, Bool_t /*overflow*/)
{
   setBin(h, i);
   return h->error();
}
template <>
double binContent<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, Int_t i, Bool_t /*overflow*/)
{
   setBin(h, i);
   return h->value();
}
template <>
double binVolume<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, Int_t bin, Bool_t /*overflow*/)
{
   double v = 1;
...
template <>
void printHistogram<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h)
{
   ...
   std::cout << "RooFitHist " << name(h) << " @ " << h << " (" << h->func()->ClassName() << " @ " << h->func()
             << "): " << n << " bins in " << dim(h) << " dimensions (";
...
void findDependantsPerBin(RooUnfolding::RooFitHist *hist, const std::vector<RooRealVar *> &allvars,
                          std::vector<RooRealVar *> &out)
{
   if (!hist)
      throw std::runtime_error("unable to find dependants of invalid histogram!");
   ...
   for (size_t i = 0; i < n; ++i) {
      ...
      if (params.size() == 0) {
         ...
         throw std::runtime_error(TString::Format("found %d parameters influencing bin %d of histogram %s!",
                                                  (int)(params.size()), (int)i, hist->name())
                                     .Data());
      }
   }
   if (!found)
...
template <class T>
void replace(T *&old, const RooAbsCollection &newlist)
{
   if (old) {
...
const char *RooFitHist::name() const
{
   return this->_func->GetName();
}
const char *RooFitHist::title() const
{
   return this->_func->GetTitle();
}
RooRealVar *RooFitHist::obs(size_t i) const
{
   if (i > this->_obs.size())
      throw std::runtime_error("attempt to access invalid observable!");
   return this->_obs[i];
}
size_t RooFitHist::dim() const
{
   return this->_obs.size();
}

const char *RooFitHist::GetName() const
{
   return this->name();
}
const char *RooFitHist::GetTitle() const
{
   return this->title();
}
void RooFitHist::Print(const char * /*opts*/) const
{
   std::cout << this->GetName() << " " << this->GetTitle() << std::endl;
   this->printHist();
}
...
RooAbsReal *RooFitHist::func() const
{
   return this->_func;
}

RooFitHist *RooFitHist::clone() const
{
   return new RooFitHist(*this);
}
...
RooFitHist::RooFitHist(RooAbsReal *f, const std::vector<RooRealVar *> &obsvec) : _func(f), _obs(obsvec)
{
   // for(auto obs:_obs) if(!obs) obs->GetName();
}
RooFitHist::RooFitHist(RooAbsReal *f, RooRealVar *obsarg) : _func(f)
{
   _obs.push_back(obsarg);
   // for(auto obs:_obs) if(!obs) obs->GetName();
}
RooFitHist::RooFitHist(RooAbsReal *f, RooRealVar *obs1, RooRealVar *obs2) : _func(f)
{
   _obs.push_back(obs1);
   _obs.push_back(obs2);
   // for(auto obs:_obs) if(!obs) obs->GetName();
}
RooFitHist::RooFitHist(RooAbsReal *f, const std::vector<RooRealVar *> &obs, const std::vector<RooRealVar *> &nps)
   : RooFitHist(f, obs)
{
   findDependantsPerBin(this, nps, this->_gamma);
}
RooFitHist::RooFitHist(RooAbsReal *f, RooRealVar *obs, const std::vector<RooRealVar *> &nps) : RooFitHist(f, obs)
{
   findDependantsPerBin(this, nps, this->_gamma);
}
RooFitHist::RooFitHist(RooAbsReal *f, RooRealVar *obs1, RooRealVar *obs2, const std::vector<RooRealVar *> &nps)
   : RooFitHist(f, obs1, obs2)
{
   findDependantsPerBin(this, nps, this->_gamma);
}
         ...
         for (int iy = 0; iy < histo->GetNbinsY(); iy++) {
            if (histo->GetNbinsZ() > 1) {
               for (int iz = 0; iz < histo->GetNbinsZ(); iz++) {
                  ...
                  double volume =
                     useIf(histo->GetXaxis()->GetBinWidth(ix + offset) * histo->GetYaxis()->GetBinWidth(iy + offset) *
                              histo->GetZaxis()->GetBinWidth(iz + offset),
                           correctDensity);
...
RooDataHist *convertTH1(const TH1 *histo, const std::vector<RooRealVar *> &vars, bool includeUnderflowOverflow,
                        bool correctDensity, double scale)
{
   return convertTH1(histo, argList(vars), includeUnderflowOverflow, correctDensity, scale);
}

RooDataHist *
convertTH1(const TH1 *histo, const RooArgList &obs, bool includeUnderflowOverflow, bool correctDensity, double scale)
{
               ...
               for (int iz = 0; iz < ::nBins(zvar); iz++) {
                  ...
                  double volume =
                     useIf(histo->GetXaxis()->GetBinWidth(ix + offset) * histo->GetYaxis()->GetBinWidth(iy + offset) *
                              histo->GetZaxis()->GetBinWidth(iz + offset),
                           correctDensity);
                  dh->add(obs, scale * histo->GetBinContent(bin) / volume,
                          TMath::Power(scale * histo->GetBinError(bin) / volume, 2));
               }
            } else {
               int bin = histo->GetBin(ix + offset, iy + offset);
               double volume =
...
RooFitHist::RooFitHist(const TH1 *hist, const std::vector<RooRealVar *> &obs, bool includeUnderflowOverflow,
                       double uncThreshold, bool correctDensity)
   : RooFitHist(hist, argList(obs), includeUnderflowOverflow, uncThreshold, correctDensity)
{
}
RooFitHist::RooFitHist(const TH1 *hist, const RooArgList &obslist, bool includeUnderflowOverflow, double uncThreshold,
                       bool correctDensity)
   : RooFitHist(convertTH1(hist, obslist, includeUnderflowOverflow, correctDensity), obslist, uncThreshold)
{
}
...
{
   RooArgList obslist;
   for (auto obs : this->_obs)
      obslist.add(*obs, true);
...
RooAbsPdf *makeHistPdf(RooDataHist *dhist, const std::vector<RooRealVar *> &obs)
{
   ...
   RooRealVar *norm =
      new RooRealVar(TString::Format("%s_norm", dhist->GetName()), dhist->GetTitle(), dhist->sumEntries());
   norm->setConstant(true);
   return new RooExtendPdf(TString::Format("%s_pdf", dhist->GetName()), dhist->GetTitle(), *hf, *norm);
}

RooFitHist::RooFitHist(RooDataHist *d, const std::vector<RooRealVar *> &obs) : _func(0), _obs(obs)
{
   this->_func = makeHistFunc(d, _obs);
}
RooFitHist::RooFitHist(RooDataHist *d, RooRealVar *obs) : _func(0)
{
   this->_obs.push_back(dynamic_cast<RooRealVar *>(obs));
   this->_func = makeHistFunc(d, _obs);
}
RooFitHist::RooFitHist(RooDataHist *d, RooRealVar *obs1, RooRealVar *obs2) : _func(0)
{
   this->_obs.push_back(dynamic_cast<RooRealVar *>(obs1));
   this->_obs.push_back(dynamic_cast<RooRealVar *>(obs2));
   this->_func = makeHistFunc(d, _obs);
}

template <>
TVectorD subtract<RooUnfolding::RooFitHist, TVectorD>(const TVectorD &orig, const RooUnfolding::RooFitHist *hist,
                                                      bool overflow, bool correctDensity)
{
   TVectorD res(orig);
   size_t n = nBins(hist, overflow);
   if (res.GetNrows() != n) {
      throw std::runtime_error("mismatch in number of bins");
   }
   for (size_t i = 0; i < n; ++i) {
...
template <>
void h2v<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, TVectorD &v, bool overflow, bool correctDensity)
{
   size_t n = nBins(h);
   v.ResizeTo(n);
   for (size_t i = 0; i < n; ++i) {
...
template <>
void h2ve<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, TVectorD &v, bool overflow, bool correctDensity)
{
   size_t n = nBins(h);
   v.ResizeTo(n);
   for (size_t i = 0; i < n; ++i) {
...
std::vector<double>
h2Xhelper(const RooAbsReal *h, const std::vector<RooRealVar *> &observables, bool overflow, bool correctDensity)
{
...
template <>
TVectorD h2v(const RooAbsReal *h, bool overflow, bool correctDensity)
{
...
template <>
TMatrixD h2m(const RooAbsReal *h, bool overflow, bool correctDensity)
{
   auto obs = getObservables(h);
   auto values = h2Xhelper(h, obs, overflow, correctDensity);
...
template <>
TVectorD h2v<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, bool overflow, bool correctDensity)
{
   TVectorD v;
...
template <>
TVectorD h2ve<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, bool overflow, bool correctDensity)
{
   TVectorD v;
...
template <>
void h2mNorm<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, TMatrixD &m,
                                       const RooUnfolding::RooFitHist *norm, bool overflow, bool correctDensity)
{
...
template <>
void h2meNorm<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, TMatrixD &m,
                                        const RooUnfolding::RooFitHist *norm, bool overflow, bool correctDensity)
{
...
template <>
TMatrixD h2mNorm<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, const RooUnfolding::RooFitHist *norm,
                                           bool overflow, bool correctDensity)
{
...
template <>
TMatrixD h2meNorm<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, const RooUnfolding::RooFitHist *norm,
                                            bool overflow, bool correctDensity)
{
...
template <>
void h2m(const RooUnfolding::RooFitHist *h, TMatrixD &m, bool overflow, bool correctDensity)
{
   h2mNorm(h, m, (const RooUnfolding::RooFitHist *)NULL, overflow, correctDensity);
}
template <>
void h2me(const RooUnfolding::RooFitHist *h, TMatrixD &m, bool overflow, bool correctDensity)
{
   h2meNorm(h, m, (const RooUnfolding::RooFitHist *)NULL, overflow, correctDensity);
}
template <>
TMatrixD h2m<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, bool overflow, bool correctDensity)
{
   // Returns Matrix of values of bins in a 2D input histogram
   TMatrixD m;
...
template <>
TMatrixD h2me<RooUnfolding::RooFitHist>(const RooUnfolding::RooFitHist *h, bool overflow, bool correctDensity)
{
   // Returns Matrix of errors of bins in a 2D input histogram
   TMatrixD m;
...
template <>
RooUnfolding::RooFitHist *clone<RooUnfolding::RooFitHist>(RooUnfolding::RooFitHist const *h)
{
   if (!h)
      return NULL;
   ...
   for (int i = 0; i < dim(h); ++i) {
      RooRealVar *rrv = findLeafServer<RooRealVar>(rr, h->obs(i)->GetName());
      obs.push_back(rrv);
   }
   ...
   if (nps.size() > 0 && nBins(h) != (int)nps.size())
      throw std::runtime_error("bin number mismatch!");
...
template std::vector<RooUnfolding::Variable<RooUnfolding::RooFitHist>>
RooUnfolding::vars<RooUnfolding::RooFitHist>(RooUnfolding::RooFitHist const *);
template void
RooUnfolding::printTable<RooUnfolding::RooFitHist>(std::ostream &, RooUnfolding::RooFitHist const *,
                                                   RooUnfolding::RooFitHist const *, RooUnfolding::RooFitHist const *,
                                                   RooUnfolding::RooFitHist const *, RooUnfolding::RooFitHist const *,
                                                   bool, RooUnfolding::ErrorTreatment, double);
...
std::vector<ObjT *> RooUnfolding::matchingObjects(const ListT *c, const char *pattern)
{
   TPRegexp re(pattern);
...
RooUnfolding::RooFitHist *
RooUnfolding::RooFitHist::asimov1DClone(bool /*correctDensity*/, TVectorD &val, TVectorD &err) const
{
   // Define x,y,z as 1st, 2nd and 3rd observable
   RooAbsArg *xvar = _obs.at(0);
...
namespace RooUnfolding {
template <>
RooUnfolding::RooFitHist *asimovClone(const RooUnfolding::RooFitHist *hist, bool correctDensity)
{
   return hist->asimovClone(correctDensity);
}
template <>
RooUnfolding::RooFitHist *
asimov1DClone(const RooUnfolding::RooFitHist *hist, bool correctDensity, TVectorD &val, TVectorD &err)
{
...
{
   return RooUnfolding::createHist<TH1, TH1>(values, errors, hist->GetName(), hist->GetTitle(),
                                             RooUnfolding::convertTH1(RooUnfolding::vars(hist)));
}
TH1 *convertTH1(const TVectorD &values, const RooUnfolding::RooFitHist *hist)
{
   return RooUnfolding::createHist<TH1, TH1>(values, hist->GetName(), hist->GetTitle(),
                                             RooUnfolding::convertTH1(RooUnfolding::vars(hist)));
}
TH2 *convertTH2(const TMatrixD &values, const TMatrixD &errors, const RooUnfolding::RooFitHist *hist)
{
   ...
   return RooUnfolding::createHist<TH2, TH2>(values, errors, hist->GetName(), hist->GetTitle(), outvars.at(0),
                                             outvars.at(1));
}

// Specialization 1: createHist<RooFitHist, RooFitHist> with TVectorT and vector of variables
template <>
RooFitHist *createHist<RooFitHist, RooFitHist>(const TVectorT<double> &vec, const char *name, const char *title,
                                               const std::vector<Variable<RooFitHist>> &x, bool overflow)
{

   // Throw a runtime error when invoked
   throw std::runtime_error(std::string("createHist<RooFitHist> with TVectorT is not supported: ") + "name: " + name +
                            ", title: " + title);
}
...
template <>
RooFitHist *
createHist<RooFitHist, RooFitHist>(const TVectorT<double> &vec, const TVectorT<double> &errvec, const char *name,
                                   const char *title, const std::vector<Variable<RooFitHist>> &x, bool overflow)
{
