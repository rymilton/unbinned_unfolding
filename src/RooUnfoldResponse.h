...
public:
   ...
   RooUnfoldResponseT(const char *name, const char *title, Hist2D *response, Hist *truth, Hist *reco, bool overflow,
                      bool density);
   // Accessors
   ...
   const TMatrixD &Mresponse(bool norm = true) const;  // Response matrix as a TMa
   const TMatrixD &Mresponse(const Hist *truth) const; // Response matrix as a TMatrixD: (row,column)=(measured,truth)
   const TMatrixD &
   Eresponse(bool norm = true) const; // Response matrix errors as a TMatrixD: (row,column)=(measured,truth)
   ...
   Hist *ApplyToTruth(const Hist *truth = 0,
                      const char *name = "AppliedResponse") const; // If argument is 0, applies itself to its own truth
...
public:
   ...
   RooUnfoldResponse(const TString &name, const TString &title)
      : RooUnfoldResponseT(name, title) {}; // named constructor

   // Special constructors

   RooUnfoldResponse(Int_t nb, Double_t xlo, Double_t xhi, const char *name = 0,
                     const char *title = 0); // constructor -  simple 1D case with same binning, measured vs truth
   RooUnfoldResponse(Int_t nm, Double_t mlo, Double_t mhi, Int_t nt, Double_t tlo, Double_t thi, const char *name = 0,
                     const char *title = 0); // constructor -  simple 1D case
   RooUnfoldResponse(const TH1 *measured, const TH1 *truth, const char *name = 0, const char *title = 0,
                     bool overflow = false); // constructor - measured and truth only used for shape
   RooUnfoldResponse(const TH1 *measured, const TH1 *truth, const TH2 *response, const char *name = 0,
                     const char *title = 0, bool overflow = false);    // create from already-filled histograms
   ...
   RooUnfoldResponse &
   Setup(Int_t nb, Double_t xlo, Double_t xhi); // set up simple 1D case with same binning, measured vs truth
   RooUnfoldResponse &
   Setup(Int_t nm, Double_t mlo, Double_t mhi, Int_t nt, Double_t tlo, Double_t thi); // set up simple 1D case
   RooUnfoldResponse &Setup(const TH1 *measured, const TH1 *truth); // set up - measured and truth only used for shape
   RooUnfoldResponse &
   Setup(const TH1 *measured, const TH1 *truth, const TH2 *response); // set up from already-filled histograms
   ...
   static Int_t
   FindBin(const TH1 *h, Double_t x, Double_t y, Double_t z); // return vector index for bin containing (x,y,z)
   ...
   Int_t Fill(Double_t xr, Double_t yr, Double_t zr, Double_t xt, Double_t yt, Double_t zt,
              Double_t w = 1.0); // Fill 3D Response Matrix
...
public:
   ...
   RooFitUnfoldResponse(
      const RooUnfoldResponseT<RooUnfolding::RooFitHist, RooUnfolding::RooFitHist> *res); // copy constructor
   virtual ~RooFitUnfoldResponse() {};                                                    // destructor
   RooFitUnfoldResponse(const char *name, const char *title, RooAbsReal *response, RooAbsReal *truth, RooAbsReal *reco,
                        RooAbsReal *fakes, const RooAbsCollection *observables, bool density = true);
   RooFitUnfoldResponse(const char *name, const char *title, RooAbsReal *response, RooAbsReal *truth, RooAbsReal *reco,
                        RooAbsReal *fakes, RooRealVar *obs_truth, RooRealVar *obs_reco, bool density = true);
   RooFitUnfoldResponse(const char *name, const char *title, RooUnfolding::RooFitHist *response,
                        RooUnfolding::RooFitHist *truth, RooUnfolding::RooFitHist *reco, bool density = true);
