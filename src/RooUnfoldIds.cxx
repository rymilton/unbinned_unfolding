...
RooUnfoldIdsT<Hist, Hist2D>::RooUnfoldIdsT(const RooUnfoldIdsT &rhs) : RooUnfoldT<Hist, Hist2D>(rhs)
{
   // Copy constructor
   Init();
   CopyData(rhs);
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
RooUnfoldIdsT<Hist, Hist2D>::RooUnfoldIdsT(const RooUnfoldResponseT<Hist, Hist2D> *res, const Hist *meas, Int_t niter,
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
RooUnfolding::Algorithm RooUnfoldIdsT<Hist, Hist2D>::GetAlgorithm() const
{
...
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::Reset()
{
   Destroy();
   Init();
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::Destroy()
{
}

//______________________________________________________________________________
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::Init()
{
   this->GetSettings();
}
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::Assign(const RooUnfoldIdsT &rhs)
{
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::CopyData(const RooUnfoldIdsT &rhs)
{
   _niter = rhs._niter;
...
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::Unfold() const
{
   ...
   if (this->_res->HasFakes()) {
      ...
      if (this->_verbose >= 1)
         std::cout << "Add truth bin for " << nfakes << " fakes" << std::endl;
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::GetCov() const
{
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
TMatrixD *RooUnfoldIdsT<Hist, Hist2D>::GetUnfoldCovMatrix(const TMatrixD *cov, Int_t ntoys, Int_t seed) const
{
   ...
   // Note that this covariance matrix will contain effects of forced normalisation if spectrum is normalised to unit
   // area.
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
TMatrixD *RooUnfoldIdsT<Hist, Hist2D>::GetAdetCovMatrix(Int_t ntoys, Int_t seed) const
{
...
template <class Hist, class Hist2D>
TVectorD *
RooUnfoldIdsT<Hist, Hist2D>::GetIDSUnfoldedSpectrum(TVectorD h_RecoMC, TVectorD h_TruthMC, TMatrixD h_2DSmear,
                                                    TVectorD h_RecoData, TVectorD h_RecoDataError, Int_t iter) const
{
   Int_t bins_reco = h_RecoData.GetNrows();

   // Sanity checks
   if (h_TruthMC.GetNrows() != bins_reco || h_RecoMC.GetNrows() != bins_reco || h_2DSmear.GetNrows() != bins_reco) {
   ...
   PerformIterations(h_RecoData, h_RecoDataError, migmatrix, bins_reco, _lambdaL, iter, _lambdaUmin, _lambdaMmin,
                     _lambdaS, &result0, &(*result));
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
Double_t RooUnfoldIdsT<Hist, Hist2D>::Probability(Double_t deviation, Double_t sigma, Double_t lambda) const
{
   if (lambda < 0.00001) {
      return 1.;
   }
...
template <class Hist, class Hist2D>
Double_t
RooUnfoldIdsT<Hist, Hist2D>::MCnormalizationCoeff(const TVectorD *vd, const TVectorD *errvd, const TVectorD *vRecmc,
                                                  const Int_t dim, const Double_t estNknownd, const Double_t Nmc,
                                                  const Double_t lambda, const TVectorD *soustr_) const
{
   ...
   for (Int_t i = 0; i < dim; i++) {
      if (((*vd)[i] - (*soustr_)[i] >= 0.) && ((*vRecmc)[i] != 0)) {
         ...
         Double_t ef = Probability(fabs((*vd)[i] - (*soustr_)[i] - estNknownd / Nmc * (*vRecmc)[i]),
                                   sqrt(pow((*errvd)[i], 2) /* + pow(estNknownd/Nmc,2)*fabs((*vRecmc)[i]) */), lambda);
...
template <class Hist, class Hist2D>
Double_t
RooUnfoldIdsT<Hist, Hist2D>::MCnormalizationCoeffIter(const TVectorD *vd, const TVectorD *errvd, const TVectorD *vRecmc,
                                                      const Int_t dim, const Double_t estNknownd, const Double_t Nmc,
                                                      const TVectorD *soustr_, Double_t lambdaN, Int_t NiterMax,
                                                      Int_t messAct) const
{
   ...
   for (Int_t i = 0; i < NiterMax; i++) {
      ...
      if (fabs((estNknownd_ - Nkd) / Nkd) < 1e-6) {
         break;
      }
      if ((i >= NiterMax - 1) && (messAct != 0)) {
         std::cout << "WARNING: a number of " << i + 1 << " steps were performed for the normalization "
                   << (estNknownd_ - Nkd) / Nkd << "  " << (estNknownd - Nkd) / Nkd << std::endl;
      }
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::IdsUnfold(const TVectorD &b, const TVectorD &errb, const TMatrixD &A, const Int_t dim,
                                            const Double_t lambda, TVectorD *soustr_, TVectorD *unf) const
{
   ...
   for (Int_t i = 0; i < dim; i++) {
      ...
      if (reco_mcN[i] != 0.0 && (b[i] - (*soustr_)[i]) > 0.0 /*&&((*b)[i]>0.)*/) {
         Double_t ef = Probability(fabs(b[i] - (*soustr_)[i] - reco_mcN[i]),
                                   sqrt(pow(errb[i], 2) /* + Nkd/Nmc*fabs((*reco_mcN)[i]) */), lambda);
         for (Int_t j = 0; j < dim; j++) {
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::ComputeSoustrTrue(const TMatrixD *A, const TVectorD *unfres,
                                                    const TVectorD *unfresErr, Int_t N, TVectorD *soustr_,
                                                    Double_t lambdaS) const
{
   ...
   for (Int_t j = 0; j < N; j++) {
      ...
      if (((*true_mcT)[j]) < 0.) {
         std::cout << "found problematic (*true_mcT)[j] " << (*true_mcT)[j] << " " << j << " " << (*unfres)[j]
                   << std::endl;
         exit(1);
      }
   }
   ...
   for (Int_t k = 0; k < 2; k++) {
      ...
      for (Int_t j = 0; j < N; j++) {
         if (((*unfres)[j] - (*soustr_)[j] > 0.) && ((*true_mcT)[j] != 0.) && ((*active)[j] > 0.)) {
            Double_t ef =
               Probability(fabs((*unfres)[j] /*- (*soustr_)[j]*/ - NkUR / Nmc * (*true_mcT)[j]),
                           sqrt(pow((*unfresErr)[j], 2) /* +pow(NkUR/Nmc,2)*fabs((*true_mcT)[j]) */), lambdaS);
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::ModifyMatrix(TMatrixD *Am, const TMatrixD *A, const TVectorD *unfres,
                                               const TVectorD *unfresErr, Int_t N, const Double_t lambdaM_,
                                               TVectorD *soustr_, const Double_t lambdaS_) const
{
   ...
   for (Int_t j = 0; j < N; j++) {
      if ((*unfres)[j] - (*soustr_)[j] > 0. && (*true_mcT)[j] != 0.) {
         Double_t ef = Probability(fabs((*unfres)[j] - (*soustr_)[j] - NkUR / Nmc * (*true_mcT)[j]),
                                   sqrt(pow((*unfresErr)[j], 2) /* +pow(NkUR/Nmc,2)*fabs((*true_mcT)[j]) */), lambdaM_);
         for (Int_t i = 0; i < N; i++) {
            ((*Am)[i][j]) +=
               ef * (((*unfres)[j] - (*soustr_)[j]) * (Nmc / NkUR) - (*true_mcT)[j]) * ((*A)[i][j]) / ((*true_mcT)[j]);
         }
      }
   }
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::PerformIterations(const TVectorD &data, const TVectorD &dataErr, const TMatrixD &A_,
                                                    const Int_t &N_, const Double_t lambdaL_, const Int_t NstepsOptMin_,
                                                    const Double_t lambdaU_, const Double_t lambdaM_,
                                                    const Double_t lambdaS_, TVectorD *unfres1IDS_,
                                                    TVectorD *unfres2IDS_) const
{
   TVectorD soustr(N_);
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
TMatrixD *RooUnfoldIdsT<Hist, Hist2D>::GetSqrtMatrix(const TMatrixD &covMat)
{
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::GenGaussRnd(TArrayD &v, const TMatrixD &sqrtMat, TRandom3 &R) const
{
...
//______________________________________________________________________________
template <class Hist, class Hist2D>
void RooUnfoldIdsT<Hist, Hist2D>::Streamer(TBuffer &R__b)
{
...
template <class Hist, class Hist2D>
RooUnfoldIdsT<Hist, Hist2D>::RooUnfoldIdsT() : RooUnfoldT<Hist, Hist2D>()
{
...
template <class Hist, class Hist2D>
RooUnfoldIdsT<Hist, Hist2D>::RooUnfoldIdsT(const char *name, const char *title) : RooUnfoldT<Hist, Hist2D>(name, title)
{
