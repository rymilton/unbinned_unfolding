...
Returns covariance matrices with conditions approximately that of the machine precision. This occasionally leads to very
...
RooUnfoldBayesT<Hist, Hist2D>::RooUnfoldBayesT(const RooUnfoldBayesT<Hist, Hist2D> &rhs) : RooUnfoldT<Hist, Hist2D>(rhs)
{
   //! Copy constructor.
   Init();
   CopyData(rhs);
...
template <class Hist, class Hist2D>
RooUnfoldBayesT<Hist, Hist2D>::RooUnfoldBayesT(const RooUnfoldResponseT<Hist, Hist2D> *res, const Hist *meas,
                                               Int_t niter, Bool_t smoothit, Bool_t handleFakes, const char *name,
                                               const char *title)
...
void RooUnfoldBayesT<Hist, Hist2D>::SetPriors(const TH1 *priors)
{
   ...
   if (nbins != _nc) {
      std::cout << "Error SetPriors: user's prior size:" << nbins << " different from response matrix size:" << _nc
                << std::endl;
   }
   _P0C.Clear();
   _P0C.ResizeTo(nbins);
...
template <class Hist, class Hist2D>
RooUnfolding::Algorithm RooUnfoldBayesT<Hist, Hist2D>::GetAlgorithm() const
{
...
template <class Hist, class Hist2D>
void RooUnfoldBayesT<Hist, Hist2D>::Init()
{
   this->_nc = this->_ne = 0;
...
template <class Hist, class Hist2D>
void RooUnfoldBayesT<Hist, Hist2D>::Reset()
{
   Init();
...
template <class Hist, class Hist2D>
void RooUnfoldBayesT<Hist, Hist2D>::Assign(const RooUnfoldBayesT<Hist, Hist2D> &rhs)
{
...
template <class Hist, class Hist2D>
void RooUnfoldBayesT<Hist, Hist2D>::CopyData(const RooUnfoldBayesT<Hist, Hist2D> &rhs)
{
...
template <class Hist, class Hist2D>
void RooUnfoldBayesT<Hist, Hist2D>::Unfold() const
{
   ...
   if (this->verbose() >= 1)
      std::cout << "Now unfolding..." << std::endl;
   unfold();
   if (this->verbose() >= 2)
      Print();
...
template <class Hist, class Hist2D>
void RooUnfoldBayesT<Hist, Hist2D>::GetCov() const
{
   int nt = this->response()->Vtruth().GetNrows();
   getCovariance();
...
template <class Hist, class Hist2D>
void RooUnfoldBayesT<Hist, Hist2D>::GetSettings() const
{
...
//-------------------------------------------------------------------------
template <class Hist, class Hist2D>
void RooUnfoldBayesT<Hist, Hist2D>::setup() const
{
   int nt = this->response()->Vtruth().GetNrows();
   ...
   if (this->_res->HasFakes() && this->_handleFakes) {
      ...
      for (int i = 0; i < nm; i++)
         this->_Nji(i, this->_nc - 1) = fakes[i];
   }
...
#ifndef OLDERRS
   if (this->_dosys != 2)
      this->_dnCidnEj.ResizeTo(this->_nc, this->_ne);
#endif
   if (this->_dosys)
      this->_dnCidPjk.ResizeTo(this->_nc, this->_ne * this->_nc);
   ...
   if (_priors == 1) {
      ...
      if (this->verbose() >= 0)
         std::cout << "Using response matrix priors" << std::endl;
   } else {
      if (this->verbose() >= 0)
         std::cout << "Using user's priors" << std::endl;
   }
   if (this->verbose() >= 1) {
...
void RooUnfoldBayesT<Hist, Hist2D>::unfold() const
{
   ...
   for (int kiter = 0; kiter < this->_niter; kiter++) {
      ...
      if (this->verbose() >= 1)
         std::cout << "Iteration : " << kiter << std::endl;
            ...
            for (int j = 0; j < this->_ne; j++) {
               for (int k = 0; k < this->_ne; k++) {
                  ...
                  for (int l = 0; l < this->_nc; l++) {
                     if (this->_P0C[l] > 0.0)
                        sum += this->_efficiencyCi[l] * this->_Mij(l, k) * this->_dnCidnEj(l, j) / this->_P0C[l];
                  }
                  ksum[k] = sum;
               }
               for (int i = 0; i < this->_nc; i++) {
      ...
      if (this->_dosys) {
         ...
         if (kiter > 0) {
            ...
            for (int i = 0; i < this->_nc; i++) {
               if (this->_P0C[i] <= 0.0)
                  continue; // skip loop: dnCidPjkUpd(i,jk) will also be 0 because this->_Mij(i,j) will be 0
...
#endif
         for (int j = 0; j < this->_ne; j++) {
            if (this->_UjInv[j] == 0.0)
               continue;
            double mbyu = this->_UjInv[j] * this->_nEstj[j];
            int j0 = j * this->_nc;
            for (int i = 0; i < this->_nc; i++) {
               double b = -mbyu * this->_Mij(i, j);
               for (int k = 0; k < this->_nc; k++)
                  this->_dnCidPjk(i, j0 + k) += b * this->_P0C[k];
...
//-------------------------------------------------------------------------
template <class Hist, class Hist2D>
void RooUnfoldBayesT<Hist, Hist2D>::getCovariance() const
{
   if (this->_dosys != 2) {
      if (this->verbose() >= 1)
         std::cout << "Calculating covariances due to number of measured events" << std::endl;
   ...
   if (this->_dosys) {
      if (this->verbose() >= 1)
         std::cout << "Calculating covariance due to unfolding matrix..." << std::endl;
...
//-------------------------------------------------------------------------
template <class Hist, class Hist2D>
void RooUnfoldBayesT<Hist, Hist2D>::smooth(TVectorD &PbarCi) const
{
...
//-------------------------------------------------------------------------
template <class Hist, class Hist2D>
double RooUnfoldBayesT<Hist, Hist2D>::getChi2(const TVectorD &prob1, const TVectorD &prob2, double nevents) const
{
   ...
   if (this->verbose() >= 2)
      std::cout << "chi2 " << n << " " << nevents << std::endl;
   for (int i = 0; i < n; i++) {
...
//-------------------------------------------------------------------------
template <class Hist, class Hist2D>
void RooUnfoldBayesT<Hist, Hist2D>::Print(Option_t *option) const
{
   RooUnfoldT<Hist, Hist2D>::Print(option);
   if (this->_nc <= 0 || this->_ne <= 0)
      return;
   ...
   if (((this->_nEstj.Sum()) != 0) || ((this->_nCi.Sum()) != 0)) {
      ...
      for (int i = 0; i < iend; i++) {
         if ((this->_nCi[i] == 0) && (this->_nEstj[i] == 0) && (this->_nEstj[i] == 0) && (this->_nbarCi[i] == 0))
            continue;
         std::cout << i << "\t" << this->_nCi[i] << "\t " << this->_nEstj[i] << "\t " << this->_nbarCi[i] << std::endl;
      }
      ...
      std::cout << "--------------------------------------------------------" << std::endl;
      std::cout << " \t" << (this->_nCi.Sum()) << "\t " << (this->_nEstj.Sum()) << "\t " << (this->_nbarCi.Sum())
                << std::endl;
      std::cout << "--------------------------------------------------------\n" << std::endl;
   }
}
...
template <class Hist, class Hist2D>
RooUnfoldBayesT<Hist, Hist2D>::RooUnfoldBayesT() : RooUnfoldT<Hist, Hist2D>()
{
